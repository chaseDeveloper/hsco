 ...
 * @Class Name   	: ACH030303ServiceImpl.java
 * @Description  	: 
 * @author       	: 박병일
 * @since        	: 2015. 9. 02.
 * @version      	: 1.0
 ...
 * </pre>  
 */
@Service("ACH030303Service")
public class ACH030303ServiceImpl extends BaseService implements ACH030303Service {
	
	protected Logger log = LoggerFactory.getLogger(ACH030303ServiceImpl.class);
	...
	public void selectList( DataSetMap tranInfo, Map<String, Object> inVar,
		...
		records = (List<Map>)baseDao.list("ACH030303DAO.selectList", inMap);		
		...
		for (int x = 0; x < listSize; x++) {
			...
			switch(rowType) {
				...
				case DataSet.ROW_TYPE_UPDATED :                              
					baseDao.update("ACH030303DAO.dtlU", map);
					
					if(map.get("CNFIRM_AT").equals("0")){
						baseDao.update("ACH030303DAO.dtl01U", map);
					}
					break;
    ...
    public void getQleEvlManList(DataSetMap tranInfo, Map<String, Object> inVar, Map<String, DataSetMap> inDataset, Map<String, Object> outVar, Map<String, DataSetMap> outDataset) throws NexaServiceException {
    	...
    	DataSetMap outDsMap = new DataSetMap();
    	outDsMap.setRowMaps((List<Map>) baseDao.list("ACH030303DAO.getQleEvlManList", inMap));
    	outDataset.put("output1", outDsMap);
    } 
}
