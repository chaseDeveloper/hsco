 ...
 * @Class Name   	: FUN040306ServiceImpl.java
 * @Description  	: 
 * @author       	: 박세현
 * @since        	: 2017. 01. 03.
 * @version      	: 1.0
...
@Service("FUN040306Service")
public class FUN040306ServiceImpl extends BaseService implements FUN040306Service {
	...
	public void selectWeekCptalPlanList( DataSetMap tranInfo, Map<String, Object> inVar,
		...
		records = (List<Map>)baseDao.list("FUN040306DAO.selectWeekCptalPlanList", inMap);
    ...
    public void WeekCptalPlanCU( DataSetMap tranInfo, Map<String, Object> inVar,
		...
		for (int i = 0; i < listSize; i++) {
			...
			Map map = list.get(i);
			int maxWeek = Integer.valueOf(map.get("MAX_WEEK").toString());
			...
			for (int j=1; j<=maxWeek; j++) {
				...
				baseDao.insert("FUN040306DAO.WeekCptalPlanCU", map);
			}
		}
    }		
    ...
    public void updateConfmAt( DataSetMap tranInfo, Map<String, Object> inVar,
		...
		for (int i = 0; i < listSize; i++) {
			...
			baseDao.insert("FUN040306DAO.updateConfmAt", map);
		}
    }		
}