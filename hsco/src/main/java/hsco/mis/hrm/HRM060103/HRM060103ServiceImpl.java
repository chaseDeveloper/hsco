...
@Service("HRM060103Service")
public class HRM060103ServiceImpl extends BaseService {
	...
	public void selectWorkEvlTrgter(DataSetMap tranInfo, Map<String, Object> inVar, Map<String, DataSetMap> inDataset, Map<String, Object> outVar, Map<String, DataSetMap> outDataset) throws NexaServiceException {
		...
		outDsMap4.setRowMaps((List<Map>) baseDao.list("HRM060103DAO.selectEvlManInfo", inMap));			// 평정자정보 조회
		...
		outDsMap1.setRowMaps((List<Map>) baseDao.list("HRM060103DAO.selectWorkEvlTrgter", inMap));		// 평정대상자 조회
		outDsMap2.setRowMaps((List<Map>) baseDao.list("HRM060103DAO.selectWorkEvlPd", inMap));			// 평정기간 조회
		outDsMap3.setRowMaps((List<Map>) baseDao.list("HRM060103DAO.selectGradDistrbTable", inMap));	// 등급분포표 조회
	...
	public void selectEvlScreEvl(DataSetMap tranInfo, Map<String, Object> inVar, Map<String, DataSetMap> inDataset, Map<String, Object> outVar, Map<String, DataSetMap> outDataset) throws NexaServiceException {
		...
		DataSetMap outDsMap3 = new DataSetMap();
		DataSetMap outDsMap4 = new DataSetMap();
		outDsMap1.setRowMaps((List<Map>) baseDao.list("HRM060103DAO.selectEvlScreEvl", inMap));			// 근무성적평정 조회
		outDsMap2.setRowMaps((List<Map>) baseDao.list("HRM060103DAO.selectWorkScreEvlOpinion", inMap));	// 근무성적평정의견 조회
		outDsMap3.setRowMaps((List<Map>) baseDao.list("HRM060103DAO.selectGradNmpr", inMap));			// 평정인원분포 조회(2020년 미만)
		outDsMap4.setRowMaps((List<Map>) baseDao.list("HRM060103DAO.selectCrevalNmprDistrb", inMap));	// 평정인원분포 조회(2020년 이상)
		outDataset.put("output1", outDsMap1);
		outDataset.put("output2", outDsMap2);
		outDataset.put("output3", outDsMap3);
		outDataset.put("output4", outDsMap4);
	...
	public void saveEvlScreEvl(DataSetMap tranInfo, Map<String, Object> inVar, Map<String, DataSetMap> inDataset, Map<String, Object> outVar, Map<String, DataSetMap> outDataset) throws NexaServiceException {
		...
		for (int x = 0; x < listSize1; x++) {
			...
			switch (rowType) {
				case DataSet.ROW_TYPE_UPDATED:
					baseDao.update("HRM060103DAO.updateEvlScreEvl", map);
					break;
			}
		}
		...
		for (int x = 0; x < listSize2; x++) {
			...
			switch (rowType) {
				case DataSet.ROW_TYPE_UPDATED:
					baseDao.update("HRM060103DAO.updateWorkScreEvlOpinion", map);
					break;
			}
		}
	}
	...
	public void selectEvlManPopup(DataSetMap tranInfo, Map<String, Object> inVar, Map<String, DataSetMap> inDataset, Map<String, Object> outVar, Map<String, DataSetMap> outDataset) throws NexaServiceException {
		...
		outDsMap.setRowMaps((List<Map>) baseDao.list("HRM060103DAO.selectEvlManPopup", inMap));
