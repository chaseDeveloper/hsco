package hsco.pms.com.PMS000006;
 ...
 * </pre>
 */
@Service("PMS000006Service")
public class PMS000006ServiceImpl extends BaseService implements PMS000006Service {
	...
	public void selectHouseCashRcipt( DataSetMap tranInfo, Map<String, Object> inVar,
		...
		records = (List<Map>)baseDao.list("PMS000006DAO.selectHouseCashRcipt", inMap);
	...
	public void selectHouseCashRciptList1( DataSetMap tranInfo, Map<String, Object> inVar,
		...
		if(inMap.get("JOB_SE").equals("RN1")){
			records = (List<Map>)baseDao.list("PMS000006DAO.selectHouseCashRciptList1RN1", inMap);
		} else if( inMap.get("JOB_SE").equals("RN2") ){
			records = (List<Map>)baseDao.list("PMS000006DAO.selectHouseCashRciptList1RN2", inMap);
		} else if(inMap.get("JOB_SE"). equals("HOU") ){
			records = (List<Map>)baseDao.list("PMS000006DAO.selectHouseCashRciptList1HOU", inMap);
		}
	...
	public void selectHouseCashRciptList2( DataSetMap tranInfo, Map<String, Object> inVar,
		...
		List <Map> records = null;
		if(inMap.get("JOB_SE").equals("RN1")){
			records = (List<Map>)baseDao.list("PMS000006DAO.selectHouseCashRciptList2RN1", inMap);
		} else if( inMap.get("JOB_SE").equals("RN2") ){
			records = (List<Map>)baseDao.list("PMS000006DAO.selectHouseCashRciptList2RN2", inMap);
		} else if(inMap.get("JOB_SE"). equals("HOU") ){
			records = (List<Map>)baseDao.list("PMS000006DAO.selectHouseCashRciptList2HOU", inMap);
		}
	...
	public void sprnt_tbpms_cash_rcipt_iu(DataSetMap tranInfo, Map<String, Object> inVar,
			Map<String, DataSetMap> inDataset, Map<String, Object> outVar,
			Map<String, DataSetMap> outDataset) throws NexaServiceException {
		DataSetMap inDsMap = (DataSetMap)inDataset.get("input1");
		Map inMap = null;
		...
		records = (List<Map>)baseDao.list("PMS000006DAO.SPRNT_TBPMS_CASH_RCIPT_IU", inMap);		
		//}
	}		
	...
	public void deleteHouseCashRcipt(DataSetMap tranInfo, Map<String, Object> inVar,
			Map<String, DataSetMap> inDataset, Map<String, Object> outVar,
			Map<String, DataSetMap> outDataset) throws NexaServiceException {
		DataSetMap inDsMap = (DataSetMap)inDataset.get("input1");
		Map inMap = null;		
		...
		if(inMap.get("JOB_SE").equals("RN1")){
			baseDao.update("PMS000006DAO.updateHouseCashRciptRN1", inMap);
		} else if( inMap.get("JOB_SE").equals("RN2") ){
			baseDao.update("PMS000006DAO.updateHouseCashRciptRN2", inMap);
		} else if(inMap.get("JOB_SE").equals("HOU") ){
			baseDao.update("PMS000006DAO.updateHouseCashRciptHOU", inMap);
		}
		baseDao.delete("PMS000006DAO.deleteHouseCashRcipt", inMap);
	}	
    ...
    public File getAtcngReqstFile(NexacroMapDTO xpDto) throws Exception {
		...
		if (inDsMap != null){
			...
			// 파일생성
			String inReqstYm = (String)inMap.get("CREAT_DE");	// 신청월
    ...
    public String getAtcngReqstFileContents(Map inMap) throws Exception
    {
    	...
    	// 헤더구성용
 //   	Map<String, Object> reqstDeInfoHeader = (Map<String, Object>)baseDao.select("PMS000006DAO.CashRciptHeader", inMap);
    	

    	// 바디구성용
    	List<Map> bodyList = null;
    	if(inMap.get("JOB_SE").equals("RN1")){
    		bodyList = (List<Map>)baseDao.list("PMS000006DAO.CashRciptBodyRN1", inMap);
		} else if( inMap.get("JOB_SE").equals("RN2") ){
			bodyList = (List<Map>)baseDao.list("PMS000006DAO.CashRciptBodyRN2", inMap);
		} else if(inMap.get("JOB_SE"). equals("HOU") ){
			bodyList = (List<Map>)baseDao.list("PMS000006DAO.CashRciptBodyHOU", inMap);
		}
    	
    	// 풋구성용
    	Map<String, Object> reqstDeInfoTail = (Map<String, Object>)baseDao.select("PMS000006DAO.CashRciptTail", inMap);
    	
    	String FileNm = "HR11" + ((String)inMap.get("CREAT_DE")).substring(4,8);			// 생성일
    	String CreatDe = (String)inMap.get("CREAT_DE");	// 최소일
    	
    //	System.out.println(":::::::inMap.creatDe :: " + inMap.get("CREAT_DE"));
    	...
    	*/
    	...
    	buffer.append(StringUtil.rpad("H", ' ', 1));		// 헤더-구분
    	buffer.append(StringUtil.rpad("0000000", ' ', 7));		// 헤더-일련번호
    	buffer.append(StringUtil.rpad("HP00000030", ' ', 10));		// 헤더-가맹점기관코드
    ...
    //	buffer.append(CRLF);
    	...
    	for(int i = 0; i < listSize; i++){
    		...
    		*/
    		buffer.append(StringUtil.rpad("D", ' ', 1));				// 바디-레코드 구분
    	...
    	//	buffer.append(StringUtil.rpad("D", ' ', 1));				// 바디-고정값
    		...
    		buffer.append(StringUtil.rpad(tcont_info, ' ', 25));		// 바디-가맹점 계약자 번호||부서코드
   ...
   // 		buffer.append(CRLF);
    	...
    	buffer.append(StringUtil.rpad("T", ' ', 1));					// 테일-데이터 구분('E'로 고정)
