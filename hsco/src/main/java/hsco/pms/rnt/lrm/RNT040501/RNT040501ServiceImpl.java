 ...
 * @Class Name   	: RNT040303ServiceImpl.java
 * @Description  	: 
 * @author       	: 이광원
 * @since        	: 2015. 7. 29.
 * @version      	: 1.0
...
@Service("RNT040501Service")
public class RNT040501ServiceImpl extends BaseService implements RNT040501Service {
	...
	public void getList(DataSetMap tranInfo, Map<String, Object> inVar,
		...
		if (inMap != null) {
			records = (List<Map>)baseDao.list("RNT040501DAO.getList", inMap);
		} else {
			records = (List<Map>)baseDao.list("RNT040501DAO.getList", inVar);
		}
	...
	public void getListR1(DataSetMap tranInfo, Map<String, Object> inVar,
		...
		if (inMap != null) {
			records = (List<Map>)baseDao.list("RNT040501DAO.getListR1", inMap);
		} else {
			records = (List<Map>)baseDao.list("RNT040501DAO.getListR1", inVar);
		}
	...
	public void listCUD(DataSetMap tranInfo, Map<String, Object> inVar,
		...
		for (int x = 0; x < list.size(); x++) {
			...
			case DataSet.ROW_TYPE_INSERTED :
				iRow += baseDao.update("RNT040501DAO.listC", map);
				break;			
			case DataSet.ROW_TYPE_UPDATED :
				iRow += baseDao.update("RNT040501DAO.listU", map);
				break;
			case DataSet.ROW_TYPE_DELETED :
				iRow += baseDao.delete("RNT040501DAO.listD1", map);
				iRow += baseDao.delete("RNT040501DAO.listD", map);
				break;
		...
		for (int x = 0; x < list.size(); x++) {
			...
			switch(rowType) {
			case DataSet.ROW_TYPE_INSERTED :
				iRow += baseDao.update("RNT040501DAO.listC1", map);
				break;			
			case DataSet.ROW_TYPE_UPDATED :
				iRow += baseDao.update("RNT040501DAO.listU1", map);
				break;
			case DataSet.ROW_TYPE_DELETED :
				iRow += baseDao.delete("RNT040501DAO.listD1", map);
				break;
	...
	public void atchCUD( DataSetMap tranInfo, Map<String, Object> inVar,
		...
		try
		{
			if(upResult != null){				
				...
				baseDao.insert("RNT040501DAO.atchCUD1", targetMap);
			}
	...
	public void atchCUD2( DataSetMap tranInfo, Map<String, Object> inVar,
		...
		try
		{
			if(upResult != null){				
				...
				String resFileSn = String.valueOf(upResult.get("FILE_SN"));
				targetMap.put("FILE_SN", resFileSn);
				baseDao.insert("RNT040501DAO.atchCUD2", targetMap);
			}
    ...
    public void atchDelete(DataSetMap tranInfo, Map<String, Object> inVar,
		...
		if (list != null){
			...
			if(fileCnt == 0)
			{
				...
				baseDao.delete("RNT040501DAO.atchDelete", inMap);
			}				
	...
	/* 수선요청 승인 */
	@SuppressWarnings("rawtypes")
	public void saveConfmse(DataSetMap tranInfo, Map<String, Object> inVar,
			Map<String, DataSetMap> inDataset, Map<String, Object> outVar,
			Map<String, DataSetMap> outDataset) throws NexaServiceException {
		...
		baseDao.insert("RNT040501DAO.listR2U", map);
