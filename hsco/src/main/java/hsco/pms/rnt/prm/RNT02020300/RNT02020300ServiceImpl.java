 ...
 * @Class Name   		: RNT02020300ServiceImpl.java
...
@Service("RNT02020300Service")
public class RNT02020300ServiceImpl extends BaseService{
	...
	public void puchasWaitrList( DataSetMap tranInfo, Map<String, Object> inVar,
		...
		if (inDsMap != null)
			inMap = inDsMap.get(0);
		List <Map> records = (List<Map>)baseDao.list("RNT02020300DAO.puchasWaitrList", inMap);				// 입주대기자 목록
	...
	public void puchasPosesnThingList( DataSetMap tranInfo, Map<String, Object> inVar,
		...
		if (inDsMap != null)
			inMap = inDsMap.get(0);
		List <Map> records = (List<Map>)baseDao.list("RNT02020300DAO.puchasPosesnThingList", inMap);		// 소유물건정보 목록
		DataSetMap outDsMap = new DataSetMap();
		outDsMap.setRowMaps(records);
		outDataset.put("output1", outDsMap);
	}
	...
	public void puchasPosesnThingCUD(DataSetMap tranInfo, Map<String, Object> inVar,
		...
		for (int x = 0; x < list.size(); x++) {
			...
			if(rowType == DataSet.ROW_TYPE_INSERTED){
				baseDao.insert("RNT02020300DAO.puchasPosesnThingC", map);	// 소유물건정보 생성
			}
			else if(rowType == DataSet.ROW_TYPE_UPDATED){
				baseDao.update("RNT02020300DAO.puchasPosesnThingU", map);	// 소유물건정보 수정
			}
			else if(rowType == DataSet.ROW_TYPE_DELETED){
				baseDao.delete("RNT02020300DAO.puchasPosesnThingD", map);	// 소유물건정보 삭제
			}

			baseDao.update("RNT02020300DAO.housePosesnAtU", map);			// 소유물건정보 수정(주태소유 유무) 0:무, 1:유
		}
	}
	...
	public void puchasPosesnThingChk( DataSetMap tranInfo, Map<String, Object> inVar,
		...
		List <Map> records = (List<Map>)baseDao.list("RNT02020300DAO.puchasPosesnThingChk", inMap);		// 소유물건정보 등록 대상자 체크
		DataSetMap outDsMap = new DataSetMap();
		outDsMap.setRowMaps(records);
		outDataset.put("output1", outDsMap);
	}
	...
	public void puchasPosesnThingCmulti( DataSetMap tranInfo, Map<String, Object> inVar,
		...
		for (int x = 0; x < list.size(); x++) {
			...
			case DataSet.ROW_TYPE_INSERTED :
				baseDao.insert("RNT02020300DAO.SPRNT_PUCHAS_POSESN_I", map);		// 소유물건 일괄등록
	...
	public void atchCUD( DataSetMap tranInfo, Map<String, Object> inVar,
		...
		try {
			if(upResult != null){
				...
				baseDao.insert("RNT02020300DAO.atchCUD", targetMap);
			}
	...
	public void ATCH_DOWN_LIST(DataSetMap tranInfo, Map<String, Object> inVar,
		...
		if (inMap != null) {
			records = (List<Map>)baseDao.list("RNT02020300DAO.ATCH_DOWN_LIST", inMap);		// 첨부파일 다운로드 목록 조회
		}
		else {
			records = (List<Map>)baseDao.list("RNT02020300DAO.ATCH_DOWN_LIST", inVar);		// 첨부파일 다운로드 목록 조회
		}
	...
	public void ATCH_FILE_DEL(DataSetMap tranInfo, Map<String, Object> inVar,
		...
		if (list != null && listSize > 0){
			...
			// 상세 목록카은트
			// 공통첨부 디테일 조회후 파일이 존재 하지 않으면 소유물관리 테이블 file_sn 값을 null로 변경
			int fileCnt = (Integer)baseDao.select("RNT02020300DAO.FILECNT", inMap);
			if(fileCnt == 0){
				baseDao.delete("RNT02020300DAO.ATCH_FILE_DEL", inMap);			// 소유물관리 FILE_SN NULL로 업데이트
			}
		}
		else{
