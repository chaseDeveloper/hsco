...
@Service("RNT02040403Service")
public class RNT02040403ServiceImpl extends BaseService{
	...
	public void selectAtcngRqestRcivList( DataSetMap tranInfo, Map<String, Object> inVar,
		...
		if (inDsMap != null)
			inMap = inDsMap.get(0);
		List <Map> records = (List<Map>)baseDao.list("RNT02040403DAO.selectAtcngRqestRcivList", inMap);
	...
	public void selectAtcngRqestRcivSum( DataSetMap tranInfo, Map<String, Object> inVar,
		...
		if (inDsMap != null)
			inMap = inDsMap.get(0);
		List <Map> records = (List<Map>)baseDao.list("RNT02040403DAO.selectAtcngRqestRcivSum", inMap);
		DataSetMap outDsMap = new DataSetMap();
		outDsMap.setRowMaps(records);
	...
	public void selectAtcngRqestRcivChk( DataSetMap tranInfo, Map<String, Object> inVar,
		...
		if (inDsMap != null)
			inMap = inDsMap.get(0);
		List <Map> records = (List<Map>)baseDao.list("RNT02040403DAO.selectAtcngRqestRcivChk", inMap);
		DataSetMap outDsMap = new DataSetMap();
		outDsMap.setRowMaps(records);
		outDataset.put("output1", outDsMap);
	}
    ...
    public void uploadAtcngReqstRcivFile(DataSetMap tranInfo, Map<String, Object> inVar, Map<String, DataSetMap> inDataset,
		...
		if(resultAt){
			...
			// 기존 해당 년월 데이터 삭제
			baseDao.delete("RNT02040403DAO.deleteAtcngRcivInfo", inMap);
			...
			for(int i = 0; i < bodyListSize; i++){
				...
				baseDao.insert("RNT02040403DAO.insertAtcngReqstRcivInfo", bodyItem);
	...
	public void saveAtcngRcivList(DataSetMap tranInfo, Map<String, Object> inVar,
        ...
        List <Map> records = (List<Map>)baseDao.list("RNT02040403DAO.selectCheck", inMap);	// 소인처리체크
        
        int chk = Integer.parseInt(records.get(0).get("CNT").toString());
        
        if(chk > 0) throw new NexaServiceException("info.처리불가","이미 소인처리된 자료입니다.");
        
        baseDao.update("RNT02040403DAO.updateAtcngRcivDemand", inMap);
        baseDao.update("RNT02040403DAO.updateAtcngRcivResult", inMap);
