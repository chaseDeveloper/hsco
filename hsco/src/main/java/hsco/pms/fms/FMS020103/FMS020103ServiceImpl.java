...
@Service("FMS020103Service")
public class FMS020103ServiceImpl extends BaseService {
	...
	public void select(DataSetMap tranInfo, Map<String, Object> inVar, Map<String, DataSetMap> inDataset, Map<String, Object> outVar, Map<String, DataSetMap> outDataset) throws NexaServiceException {
		...
		outDsMap.setRowMaps((List<Map>) baseDao.list("FMS020103DAO.selectFcltsList", inMap));
	...
	public void selectSub(DataSetMap tranInfo, Map<String, Object> inVar, Map<String, DataSetMap> inDataset, Map<String, Object> outVar, Map<String, DataSetMap> outDataset) throws NexaServiceException {
		...
		outDsMap1.setRowMaps((List<Map>) baseDao.list("FMS020103DAO.selectReprsntFcltsManage", inMap));
		outDsMap2.setRowMaps((List<Map>) baseDao.list("FMS020103DAO.selectReprsntFcltsBrkwtr", inMap));
		...
		for (int x = 0; x < list1.size(); x++) {
			...
			switch (rowType) {
				case DataSet.ROW_TYPE_DELETED:
					baseDao.delete("FMS020103DAO.deleteReprsntFcltsManage", map);
					baseDao.delete("FMS020103DAO.deleteReprsntFcltsBrkwtr", map);
					break;
			}
		}
		...
		for (int x = 0; x < list2.size(); x++) {
			...
			switch (rowType) {
				case DataSet.ROW_TYPE_INSERTED:
					fcltsManageSn = (String) baseDao.select("FMS020103DAO.getFcltsManageSn", map);
					map.put("FCLTS_MANAGE_SN", fcltsManageSn);
					baseDao.insert("FMS020103DAO.insertReprsntFcltsManage", map);
					break;
				case DataSet.ROW_TYPE_UPDATED:
					baseDao.update("FMS020103DAO.updateReprsntFcltsManage", map);
					break;
			}
		}
		...
		for (int x = 0; x < list3.size(); x++) {
			...
			switch (rowType) {
				case DataSet.ROW_TYPE_INSERTED:
					...
					baseDao.insert("FMS020103DAO.insertReprsntFcltsBrkwtr", map);
					break;
				case DataSet.ROW_TYPE_UPDATED:
					baseDao.update("FMS020103DAO.updateReprsntFcltsBrkwtr", map);
					break;
			}
		}
	}
