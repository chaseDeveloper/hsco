...
@Service("FMS030202Service")
public class FMS030202ServiceImpl extends BaseService {
	...
	public void selectFcltsList(DataSetMap tranInfo, Map<String, Object> inVar, Map<String, DataSetMap> inDataset, Map<String, Object> outVar, Map<String, DataSetMap> outDataset) throws NexaServiceException {
		...
		outDsMap.setRowMaps((List<Map>) baseDao.list("FMS030202DAO.selectFcltsList", inMap));
	 ...
	 */
	@SuppressWarnings({ "rawtypes", "unchecked" })
	public void selectMendngReinfcAcmslt(DataSetMap tranInfo, Map<String, Object> inVar, Map<String, DataSetMap> inDataset, Map<String, Object> outVar, Map<String, DataSetMap> outDataset) throws NexaServiceException {
		Map inMap = null;
		DataSetMap inDsMap1 = (DataSetMap) inDataset.get("input1");
		if (inDsMap1 != null) {
			inMap = inDsMap1.get(0);
		...
		DataSetMap outDsMap = new DataSetMap();
		outDsMap.setRowMaps((List<Map>) baseDao.list("FMS030202DAO.selectMendngReinfcAcmslt", inMap));
		outDataset.put("output1", outDsMap);
	}
		...
		for (int x = 0; x < list1.size(); x++) {
			...
			switch (rowType) {
				case DataSet.ROW_TYPE_INSERTED:
					baseDao.insert("FMS030202DAO.insertMendngReinfcAcmslt", map);
					break;
				case DataSet.ROW_TYPE_UPDATED:
					baseDao.update("FMS030202DAO.updateMendngReinfcAcmslt", map);
					break;
				case DataSet.ROW_TYPE_DELETED:
					baseDao.delete("FMS030202DAO.deleteMendngReinfcAcmslt", map);
					break;
			}
		}
	}
