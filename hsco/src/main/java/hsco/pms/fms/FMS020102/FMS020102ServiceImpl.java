package hsco.pms.fms.FMS020102;
...
@Service("FMS020102Service")
public class FMS020102ServiceImpl extends BaseService {
	...
	public void select(DataSetMap tranInfo, Map<String, Object> inVar, Map<String, DataSetMap> inDataset, Map<String, Object> outVar, Map<String, DataSetMap> outDataset) throws NexaServiceException {
		...
		outDsMap.setRowMaps((List<Map>) baseDao.list("FMS020102DAO.selectFcltsList", inMap));
	...
	public void selectSub(DataSetMap tranInfo, Map<String, Object> inVar, Map<String, DataSetMap> inDataset, Map<String, Object> outVar, Map<String, DataSetMap> outDataset) throws NexaServiceException {
		...
		outDsMap1.setRowMaps((List<Map>) baseDao.list("FMS020102DAO.selectReprsntFcltsManage", inMap));
		outDsMap2.setRowMaps((List<Map>) baseDao.list("FMS020102DAO.selectReprsntFcltsMrngFclty", inMap));
		...
		for (int x = 0; x < list1.size(); x++) {
			...
			switch (rowType) {
				case DataSet.ROW_TYPE_DELETED:
					baseDao.delete("FMS020102DAO.deleteReprsntFcltsManage", map);
					baseDao.delete("FMS020102DAO.deleteReprsntFcltsMrngFclty", map);
					break;
			}
		}
		...
		for (int x = 0; x < list2.size(); x++) {
			...
			switch (rowType) {
				case DataSet.ROW_TYPE_INSERTED:
					fcltsManageSn = (String) baseDao.select("FMS020102DAO.getFcltsManageSn", map);
					map.put("FCLTS_MANAGE_SN", fcltsManageSn);
					baseDao.insert("FMS020102DAO.insertReprsntFcltsManage", map);
					break;
				case DataSet.ROW_TYPE_UPDATED:
					baseDao.update("FMS020102DAO.updateReprsntFcltsManage", map);
					break;
			}
		}
		...
		for (int x = 0; x < list3.size(); x++) {
			...
			switch (rowType) {
				case DataSet.ROW_TYPE_INSERTED:
					...
					baseDao.insert("FMS020102DAO.insertReprsntFcltsMrngFclty", map);
					break;
				case DataSet.ROW_TYPE_UPDATED:
					baseDao.update("FMS020102DAO.updateReprsntFcltsMrngFclty", map);
					break;
			}
		}
	}
