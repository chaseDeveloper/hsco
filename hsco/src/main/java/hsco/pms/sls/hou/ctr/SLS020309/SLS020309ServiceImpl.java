...
@Service("SLS020309Service")
public class SLS020309ServiceImpl extends BaseService implements SLS020309Service {
	...
	public void selectLadReceiveList( DataSetMap tranInfo, Map<String, Object> inVar,
		...
		List<Map> headDateList = (List<Map>) baseDao.list("SLS020309DAO.selectHeadDateList", inMap);
		//inMap.put("PAY_MONTH_LIST", headDateList);	// 조회 월 목록 2016.04.29 주석처리
				
		// 로우 조회용
		List<Map> ladReceiveList = (List<Map>)baseDao.list("SLS020309DAO.selectLadReceiveList", inMap);
		...
		if(headDateList == null){
			...
			for(int i = 0 ; i < ladReceiveList.size() ; i++){
				...
				for(int z = 0 ; z < headDateList.size() ; z++ ){
					...
					Map rsltColMap = (Map)baseDao.select("SLS020309DAO.selectLadReceiveList2", inMap); // cell 값 조회 
					// cell 값 설정
					if(rsltColMap == null){
						 rowMap.put(colName , ""); 
					}
	...
	public void selectHeadDateList( DataSetMap tranInfo, Map<String, Object> inVar,
		...
		List <Map> records = (List<Map>)baseDao.list("SLS020309DAO.selectHeadDateList", inMap);
