package hsco.pms.prj.PRJ010114;
 ...
 * @Class Name   	: PRJ010114ServiceImpl.java
 * @Description  	: 
 * @author       	: 이수지
 * @since        	: 2017. 11. 13.
 * @version      	: 1.0
...
@Service("PRJ010114Service")
public class PRJ010114ServiceImpl extends BaseService implements PRJ010114Service {
    ...
    public void selectDsMain(DataSetMap tranInfo, Map<String, Object> inVar, Map<String, DataSetMap> inDataset, Map<String, Object> outVar, Map<String, DataSetMap> outDataset) throws NexaServiceException {
        ...
        outDsMap.setRowMaps((List<Map>) baseDao.list("PRJ010114DAO.selectDs_Main", inMap));
    ...
    public void selectDsSub(DataSetMap tranInfo, Map<String, Object> inVar, Map<String, DataSetMap> inDataset, Map<String, Object> outVar, Map<String, DataSetMap> outDataset) throws NexaServiceException {
        ...
        DataSetMap outDsMap = new DataSetMap();
        outDsMap.setRowMaps((List<Map>) baseDao.list("PRJ010114DAO.selectDsSub", inMap));
        outDataset.put("output1", outDsMap);
    }
    ...
    public void saveDsMain(DataSetMap tranInfo, Map<String, Object> inVar, Map<String, DataSetMap> inDataset, Map<String, Object> outVar, Map<String, DataSetMap> outDataset) throws NexaServiceException {
        ...
        for (int x = 0; x < list1.size(); x++) {
            ...
            case DataSet.ROW_TYPE_INSERTED:
                map.put("EXE_REQ_SN", baseDao.select("PRJ010112DAO.createExeEvlReq"));
                baseDao.insert("PRJ010114DAO.insertDs_Main", map);
                break;
            case DataSet.ROW_TYPE_UPDATED:
                baseDao.update("PRJ010114DAO.updateDs_Main", map);
                break;
            case DataSet.ROW_TYPE_DELETED:
                baseDao.delete("PRJ010114DAO.deleteDs_Sub", map);
                baseDao.delete("PRJ010114DAO.deleteDs_Main", map);
                break;
            }
        }
        ...
        for (int x = 0; x < list2.size(); x++) {
            ...
            switch (rowType) {
            case DataSet.ROW_TYPE_INSERTED:
                baseDao.insert("PRJ010114DAO.insertDs_Sub", map);
                break;
            case DataSet.ROW_TYPE_UPDATED:
                baseDao.update("PRJ010114DAO.updateDs_Sub", map);
                break;
            case DataSet.ROW_TYPE_DELETED:
                baseDao.delete("PRJ010114DAO.deleteDs_Sub", map);
                break;
    ...
    public void selectCtgryFile(DataSetMap tranInfo, Map<String, Object> inVar, Map<String, DataSetMap> inDataset, Map<String, Object> outVar, Map<String, DataSetMap> outDataset) throws NexaServiceException {
        ...
        DataSetMap outDsMap = new DataSetMap();
        outDsMap.setRowMaps((List<Map>) baseDao.list("PRJ010114DAO.selectCtgryFile", inMap));
        outDataset.put("output1", outDsMap);
    }
            ...
            switch (rowType) {
                ...
                case DataSet.ROW_TYPE_UPDATED:
                    ...
                    if (StringUtils.isEmpty(ctgryFileId) || "".equals(ctgryFileId)) {
                        ctgryFileId = (String) baseDao.select("PRJ010114DAO.getCtgryFileId", map);
                        map.put("CTGRY_FILE_ID", ctgryFileId);
                        baseDao.insert("PRJ010114DAO.insertCtgryFile", map);
                        
                        String docSn = (String) map.get("DOC_SN");
                        if (StringUtils.isEmpty(docSn) || "".equals(docSn)) {
                            docSn = (String) baseDao.select("PRJ010114DAO.getDocSn", map);
                            map.put("DOC_SN", docSn);
                            baseDao.update("PRJ010114DAO.updateDocSn", map);        // 문서번호 갱신(부모테이블)
                        }
                        baseDao.insert("PRJ010114DAO.insertDocRgs", map);       // 문서 등록 추가
                    }

                    
                    break;
                case DataSet.ROW_TYPE_DELETED:
                    baseDao.delete("PRJ010114DAO.deleteCtgryFile", map);
                    baseDao.delete("PRJ010114DAO.deleteDocRgs", map);
                    /*

                    map.put("DOC_SN", "");

                    // 문서번호 갱신(부모테이블)
                    baseDao.update("PRJ010114DAO.updateDocSn", map);
                    */
                    break;
            }
            resultList.add(map);
