 ...
 * @Class Name   	: PRJ010112ServiceImpl.java
 * @Description  	: 
 * @author       	: 이수지
 * @since        	: 2017. 10. 31.
 * @version      	: 1.0
...
@Service("PRJ010112Service")
public class PRJ010112ServiceImpl extends BaseService implements PRJ010112Service {
    ...
    public void selectDsMain(DataSetMap tranInfo, Map<String, Object> inVar, Map<String, DataSetMap> inDataset, Map<String, Object> outVar, Map<String, DataSetMap> outDataset) throws NexaServiceException {
        ...
        outDsMap.setRowMaps((List<Map>) baseDao.list("PRJ010112DAO.selectDs_Main", inMap));
    ...
    public void saveDsMain(DataSetMap tranInfo, Map<String, Object> inVar, Map<String, DataSetMap> inDataset, Map<String, Object> outVar, Map<String, DataSetMap> outDataset) throws NexaServiceException {
        ...
        for (int x = 0; x < list1.size(); x++) {
            ...
            case DataSet.ROW_TYPE_INSERTED:
                map.put("EXE_PLN_SN", baseDao.select("PRJ010112DAO.createExePln"));
                baseDao.insert("PRJ010112DAO.insertDs_Main", map);
                break;
            case DataSet.ROW_TYPE_UPDATED:
                baseDao.update("PRJ010112DAO.updateDs_Main", map);
                break;
            case DataSet.ROW_TYPE_DELETED:
                baseDao.delete("PRJ010112DAO.deleteDs_Main", map);
                break;
            }
        }
    }
    ...
    public void selectCtgryFile(DataSetMap tranInfo, Map<String, Object> inVar, Map<String, DataSetMap> inDataset, Map<String, Object> outVar, Map<String, DataSetMap> outDataset) throws NexaServiceException {
        ...
        DataSetMap outDsMap = new DataSetMap();
        outDsMap.setRowMaps((List<Map>) baseDao.list("PRJ010112DAO.selectCtgryFile", inMap));
        outDataset.put("output1", outDsMap);
    }
            ...
            switch (rowType) {
                ...
                case DataSet.ROW_TYPE_UPDATED:
                    ...
                    if (StringUtils.isEmpty(ctgryFileId) || "".equals(ctgryFileId)) {
                        ctgryFileId = (String) baseDao.select("PRJ010112DAO.getCtgryFileId", map);
                        map.put("CTGRY_FILE_ID", ctgryFileId);
                        baseDao.insert("PRJ010112DAO.insertCtgryFile", map);
                        
                        String docSn = (String) map.get("DOC_SN");
                        if (StringUtils.isEmpty(docSn) || "".equals(docSn)) {
                            docSn = (String) baseDao.select("PRJ010112DAO.getDocSn", map);
                            map.put("DOC_SN", docSn);
                            baseDao.update("PRJ010112DAO.updateDocSn", map);        // 문서번호 갱신(부모테이블)
                        }
                        baseDao.insert("PRJ010112DAO.insertDocRgs", map);       // 문서 등록 추가
                    }

                    
                    break;
                case DataSet.ROW_TYPE_DELETED:
                    baseDao.delete("PRJ010112DAO.deleteCtgryFile", map);
                    baseDao.delete("PRJ010112DAO.deleteDocRgs", map);
                    /*

                    map.put("DOC_SN", "");

                    // 문서번호 갱신(부모테이블)
                    baseDao.update("PRJ010112DAO.updateDocSn", map);
                    */
                    break;
            }
            resultList.add(map);
