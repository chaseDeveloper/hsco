...
public class DataSetMap {
	...
	public void convertDataSet2DataSetMap(DataSet ds) {
		...
		for ( int j = 0; j < ds.getRowCount(); j ++ ){		//Dataset의 record 반복
			...
			map.put(NexacroConstant.DATASET_ROW_TYPE,Integer.valueOf(rowType1)); // 성능개선 by jdkim
			rowMaps.add(map);
		}
		...
		for (int i = 0; i < removeRowCount; i++) {
			...
			map.put(NexacroConstant.DATASET_ROW_TYPE, Integer.valueOf(DataSet.ROW_TYPE_DELETED));  // 성능개선 by jdkim
	...
	public void convertDataSet2DataSetMap(DataSet ds, Map mapUserInfo, Map mapPageInfo) {
		...
		for ( int j = 0; j < ds.getRowCount(); j ++ ){		//Dataset의 record 반복
			...
			int rowType1 = ds.getRowType(j);
			map.put(NexacroConstant.DATASET_ROW_TYPE,Integer.valueOf(rowType1));
		...
		for (int i = 0; i < removeRowCount; i++) {
			map = new HashMap<String, Object>();
			map.put(NexacroConstant.DATASET_ROW_TYPE, Integer.valueOf(DataSet.ROW_TYPE_DELETED));
