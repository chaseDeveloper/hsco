...
public class NexacroMapDTO implements Serializable {
	...
	/**
	 * Request 정보
	 * - 파일업로드등 Request 정보를 이용할 뎡우를 대비하여 추가함 
	 * @param request
	 */
	...
	public void setVariableMap(VariableList vList, DataSetList dsList) {
		...
		Iterator entries = mapParamInfo.entrySet().iterator();
		...
		if (dsTransInfo != null) {
			...
			for (int i = 0; i < dsTransInfo.getRowCount(); i++) {
				...
				for (int j = 0; j < dsTransInfo.getColumnCount(); j++) {
					//System.out.println("{"+dsTransInfo.getString(i, 0)+", "+ dsTransInfo.getString(i, 1)+"}");
...
//			System.out.println("dsName="+str1);
//		}
