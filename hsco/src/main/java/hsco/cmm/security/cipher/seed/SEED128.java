...
/************************** SEED encrtyption function *************************/
	...
	// ++ soon �? ����� ���߾� ��ȣȭ �ϵ��� Wrapped Method�� ��
	public static void Encrypt(byte pbData[], int pdwRoundKey[], byte outData[])
	{		
		int blockCount = pbData.length%blockSize > 0 ? pbData.length/blockSize +1 : pbData.length/blockSize;
		
		//System.out.print("count: " + count + "\n");
		
		byte encData[] = new byte[blockSize*blockCount]; 	// ���ڵ� ��� ���� ���� ����
				
		//System.out.print("enc size: " + blockSize*blockCount + ", blockcount: "+blockCount+"\n");
		
		for(int i=0;i<blockCount;i++)	{
			byte _data[] = new byte[blockSize];
			byte _outData[] = new byte[blockSize];
			System.arraycopy(pbData, i*blockSize, _data, 0, blockSize);	// ��ȣȭ �?�� �����
			
			//System.out.print("i:"+i+"\n");
			/*			
			for (int j=0; i<_data.length; j++)	System.out.print(Integer.toHexString(0xff&_data[j])+" ");
			System.out.print("\n\n");
			*/
			
			SeedEncrypt(_data, pdwRoundKey, _outData);
			//System.out.print("_data length:"+_data.length+", _outData length:"+_outData.length+"\n");
			//for (int j=0; j<_outData.length; j++)	System.out.print(Integer.toHexString(0xff&_outData[j])+" ");
			//System.out.print("\n\n");
			
			System.arraycopy(_outData, 0, encData, i*blockSize, blockSize);	
		}		
		
		System.arraycopy(encData, 0, outData, 0, blockCount*blockSize);	
		//System.out.print("encData length:"+encData.length+"\n");
		//return encData;		
	}
	
	// ++ soon �? ����� ���߾� ��ȣȭ �ϵ��� Wrapped Method�� ��
	public static void Decrypt(byte pbData[], int pdwRoundKey[], byte outData[])
	{		
		int blockCount = pbData.length%blockSize > 0 ? pbData.length/blockSize +1 : pbData.length/blockSize;
		
		//System.out.print("count: " + count + "\n");
		
		byte decData[] = new byte[blockSize*blockCount]; 	// ���ڵ� ��� ���� ���� ����
		...
		for(int i=0;i<blockCount;i++)	{
			...
			*/
			...
			SeedDecrypt(_data, pdwRoundKey, _outData);
			//System.out.print("_data length:"+_data.length+", _outData length:"+_outData.length+"\n");
			//for (int j=0; j<_outData.length; j++)	System.out.print(Integer.toHexString(0xff&_outData[j])+" ");
			//System.out.print("\n\n");
			
			System.arraycopy(_outData, 0, decData, i*blockSize, blockSize);	
		}		
		
		System.arraycopy(decData, 0, outData, 0, blockCount*blockSize);	
		//System.out.print("encData length:"+encData.length+"\n");
		//return encData;		
	}
					...
					byte outData[])					// [out] encrypted data
	{
        ...
        K[0] = pdwRoundKey[nCount++];		K[1] = pdwRoundKey[nCount++];       SeedRound(L0, L1, R0, R1, K); /*   1 */
        K[0] = pdwRoundKey[nCount++];		K[1] = pdwRoundKey[nCount++];       SeedRound(R0, R1, L0, L1, K); /*   2 */
        K[0] = pdwRoundKey[nCount++];		K[1] = pdwRoundKey[nCount++];       SeedRound(L0, L1, R0, R1, K); /*   3 */
        K[0] = pdwRoundKey[nCount++];		K[1] = pdwRoundKey[nCount++];       SeedRound(R0, R1, L0, L1, K); /*   4 */
        K[0] = pdwRoundKey[nCount++];		K[1] = pdwRoundKey[nCount++];       SeedRound(L0, L1, R0, R1, K); /*   5 */
        K[0] = pdwRoundKey[nCount++];		K[1] = pdwRoundKey[nCount++];       SeedRound(R0, R1, L0, L1, K); /*   6 */
        K[0] = pdwRoundKey[nCount++];		K[1] = pdwRoundKey[nCount++];       SeedRound(L0, L1, R0, R1, K); /*   7 */
        K[0] = pdwRoundKey[nCount++];		K[1] = pdwRoundKey[nCount++];       SeedRound(R0, R1, L0, L1, K); /*   8 */
        K[0] = pdwRoundKey[nCount++];		K[1] = pdwRoundKey[nCount++];       SeedRound(L0, L1, R0, R1, K); /*   9 */
        K[0] = pdwRoundKey[nCount++];		K[1] = pdwRoundKey[nCount++];       SeedRound(R0, R1, L0, L1, K); /*  10 */
        K[0] = pdwRoundKey[nCount++];		K[1] = pdwRoundKey[nCount++];       SeedRound(L0, L1, R0, R1, K); /*  11 */
        K[0] = pdwRoundKey[nCount++];		K[1] = pdwRoundKey[nCount++];       SeedRound(R0, R1, L0, L1, K); /*  12 */
        K[0] = pdwRoundKey[nCount++];		K[1] = pdwRoundKey[nCount++];       SeedRound(L0, L1, R0, R1, K); /*  13 */
        K[0] = pdwRoundKey[nCount++];		K[1] = pdwRoundKey[nCount++];       SeedRound(R0, R1, L0, L1, K); /*  14 */
        K[0] = pdwRoundKey[nCount++];		K[1] = pdwRoundKey[nCount++];       SeedRound(L0, L1, R0, R1, K); /*  15 */
        K[0] = pdwRoundKey[nCount++];		K[1] = pdwRoundKey[nCount++];       SeedRound(R0, R1, L0, L1, K); /*  16 */
...
// Same as encrypt, except that round keys are applied in reverse order
					...
					byte outData[])					// [out] data to be encrypted
	{
		...
		K[1] = pdwRoundKey[nCount--];		K[0] = pdwRoundKey[nCount--];       SeedRound(L0, L1, R0, R1, K); /*   1 */
 		K[1] = pdwRoundKey[nCount--];		K[0] = pdwRoundKey[nCount--];       SeedRound(R0, R1, L0, L1, K); /*   2 */
 		K[1] = pdwRoundKey[nCount--];		K[0] = pdwRoundKey[nCount--];       SeedRound(L0, L1, R0, R1, K); /*   3 */
 		K[1] = pdwRoundKey[nCount--];		K[0] = pdwRoundKey[nCount--];       SeedRound(R0, R1, L0, L1, K); /*   4 */
 		K[1] = pdwRoundKey[nCount--];		K[0] = pdwRoundKey[nCount--];       SeedRound(L0, L1, R0, R1, K); /*   5 */
 		K[1] = pdwRoundKey[nCount--];		K[0] = pdwRoundKey[nCount--];       SeedRound(R0, R1, L0, L1, K); /*   6 */
 		K[1] = pdwRoundKey[nCount--];		K[0] = pdwRoundKey[nCount--];       SeedRound(L0, L1, R0, R1, K); /*   7 */
 		K[1] = pdwRoundKey[nCount--];		K[0] = pdwRoundKey[nCount--];       SeedRound(R0, R1, L0, L1, K); /*   8 */
 		K[1] = pdwRoundKey[nCount--];		K[0] = pdwRoundKey[nCount--];       SeedRound(L0, L1, R0, R1, K); /*   9 */
 		K[1] = pdwRoundKey[nCount--];		K[0] = pdwRoundKey[nCount--];       SeedRound(R0, R1, L0, L1, K); /*  10 */
 		K[1] = pdwRoundKey[nCount--];		K[0] = pdwRoundKey[nCount--];       SeedRound(L0, L1, R0, R1, K); /*  11 */
 		K[1] = pdwRoundKey[nCount--];		K[0] = pdwRoundKey[nCount--];       SeedRound(R0, R1, L0, L1, K); /*  12 */
 		K[1] = pdwRoundKey[nCount--];		K[0] = pdwRoundKey[nCount--];       SeedRound(L0, L1, R0, R1, K); /*  13 */
 		K[1] = pdwRoundKey[nCount--];		K[0] = pdwRoundKey[nCount--];       SeedRound(R0, R1, L0, L1, K); /*  14 */
 		K[1] = pdwRoundKey[nCount--];		K[0] = pdwRoundKey[nCount--];       SeedRound(L0, L1, R0, R1, K); /*  15 */
 		K[1] = pdwRoundKey[nCount--];		K[0] = pdwRoundKey[nCount];         SeedRound(R0, R1, L0, L1, K); /*  16 */
