...
public class BatchSchdul extends ComDefaultVO implements Serializable {
	...
	public void makeExecutSchdul(List<BatchSchdulDfk> dfkSeList) {
		...
		if (this.executCycle.equals("02")) {
			...
			if (dfkSeList.size() != 0) {
				for (int i = 0; i < dfkSeList.size(); i++) {
					...
					buf.append(dfkSeList.get(i).getExecutSchdulDfkSeNm());
		...
		buf.append( this.executSchdulHour + ":" + this.executSchdulMnt + ":" + this.executSchdulSecnd);
		// 값지정.
		//this.executSchdul = executSchdul;
		this.executSchdul = buf.toString();
	}
	...
	public String toCronExpression() {
		...
		if (this.executCycle.equals("02")) {
			...
			for (int i = 0; i < this.executSchdulDfkSes.length; i++) {
				...
				buf.append(this.executSchdulDfkSes[i]);
				
			}
			cronExpression = cronExpression + " " + buf.toString();
		} else {
			// 이외의 경우 "?" 출력
			cronExpression = cronExpression + " " + "?";
		}
