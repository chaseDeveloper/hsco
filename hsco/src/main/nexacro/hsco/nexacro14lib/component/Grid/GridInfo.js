...
if (!nexacro.GridFormat) {
	...
	eval(nexacro._createValueAttributeEvalStr("_pGridBandStyle", "celllinespace"));
	...
	_pGridBandStyle.__get_custom_style_value = function () {
		...
		if (this.celllinespace && !this.celllinespace._is_empty) {
			val += "celllinespace:" + this.celllinespace._value + "; ";
		}
	...
	eval(nexacro._createValueAttributeEvalStr("_pGridCellStyle", "linespace"));
	...
	nexacro.GridCellInfo = function (id, band, grid, type, idx) {
		...
		this.imagestretch = new nexacro.BindableValue("none");
	};
	...
	_pGridCellInfo.on_update_style_linespace = function () {
	};
			...
			if (dataset == null) {
				...
				if (propobj._bindtype == 1) {
					...
					if ((typeof exprfn) == "function") {
						...
						var rowpos = (bandinfo.bandtype == "body") ? rowidx : grid._rowposition;

						val = exprfn.call(this, rowidx, rowpos, grid, dataset);
						if (val) {
							val = val.toString();
						}
					}
	...
	_pGridCellInfo._searchStyleValue = function (propid, styleid, rowidx, odd, selected, pseudo, usecache, exprbind, lastcache) {
		...
		if (propid == "background") {
		...
		else if (propid == "linespace") {
			var linespace = style.linespace;
			result = this._resultStyleProc(styleid, propid, linespace, exprbind, rowidx, usecache);

			if (this.isStyleEmpty(result)) {
				if (style != enablestyle) {
					linespace = enablestyle.linespace;
					result = this._resultStyleProc(styleid, propid, linespace, exprbind, rowidx, usecache, lastcache);
				}
			}
			else {
				return result;
			...
			if (this.isStyleEmpty(result)) {
				if (this.cssclass) {
					linespace = this._findcss_ctrl._find_pseudo_obj("linespace", pseudo, "linespace");
					result = this._resultStyleProc(styleid, propid, linespace, exprbind, rowidx, usecache);
				}
			}
			else {
			...
			if (this.isStyleEmpty(result)) {
				linespace = bandinfo._find_gridpseudo_obj("celllinespace", pseudo, this.celltype);
				result = this._resultStyleProc(styleid, propid, linespace, exprbind, rowidx, usecache, lastcache);
			}
			else {
				return result;
	...
	_pGridCellInfo._query_pseudo_control = function (control, rowidx, controlProp, styleProp, pseudo, returnType, childctrl, odd, selected) {
		...
		if (styleProp != "color" && styleProp != "font" && styleProp != "letterspace" && styleProp != "align") {
			if (childctrl) {
		...
		if (this.isStyleEmpty(result)) {
			if (controlProp == "controlalign") {
				...
				if (this.isStyleEmpty(result)) {
					propobj = this.parent.style.cellalign;
	...
	_pGridCellInfo._query_pseudo_padding = function (rowidx, pseudo) {
		...
		var org_padding = this._stylecache[styleid];
		...
		if (!org_padding || org_padding._bindtype == 0) {
			this._curpadding = org_padding;
		}
		else {
	...
	_pGridCellInfo._query_pseudo_linespace = function (rowidx, pseudo) {
		var styleid = ["linespace", pseudo].join("");
		var linespace = this._stylecache[styleid];

		if (!linespace || linespace._bindtype > 0) {
			linespace = this._searchStyleValue("linespace", styleid, rowidx, false, false, pseudo, true, null, false);
		}
	...
	_pGridCellInfo._query_pseudo_align = function (rowidx, displayType, pseudo) {
		...
		if (!align || align._is_empty) {
			...
			var temp_align = new nexacro.Style_align(align._value);
			...
			if (!temp_align.halign) {
				temp_align.set_halign(align2.halign);
			}
			if (!temp_align.valign) {
				temp_align.set_valign(align2.valign);
			}
			this._addStyleCache(styleid, align);
			return temp_align;
		}
	...
	_pGridCellInfo.set_combotype = function (v) {
		if (v != this.combotype._value) {
			this.combotype._set_enumval(v, ["dropdown", "search", "filter", "filterlike"]);
			this.grid._clearBindTypeFlag();
		}
	};
	...
	_pGridCellInfo.set_editfilter = function (v) {
		...
		for (var i = 0, n = strval.length; i < n; i++) {
			...
			for (var j = 0, arr_len = arrstr.length; j < arr_len; j++) {
	...
	_pGridCellInfo.set_treestartlevel = function (v) {
		if (v != this.treestartlevel._value) {
			...
			this.grid._clearBindTypeFlag();
			this.grid._rootlevel = Math.max(this.treestartlevel._value, this.grid._rootlevel);
	...
	_pGridCellInfo.set_imagestretch = function (v) {
		if (v != this.imagestretch._value) {
			this.imagestretch._set(v);
		}
	};
	...
	_pGridCellInfo.__get_view_zeromask = function (mask) {
		...
		for (i = intlast - 1; i >= 0; i--) {
			var ch = mask.charAt(i);
			if (ch == "0" && (integer == 0 || nineidx < i)) {
				integer++;
			}
			else if (integer > 0 && ch == "#") {
				integer++;
			}
			else if (ch == "9" && integer > 0 && nineidx <= i) {
				integer++;
			}
		}
			...
			if (this.__isMask(maskCh) || maskCh == "\\" || maskCh == "\"" || maskCh == "\'") {
				...
				if (n < valueLen) {
					while (n < valueLen) {
						...
						if (ch == " " && this.editclipmode._value == "includespace") {
							text += maskchar;
							break;
						}
	...
	_pGridCellInfo._getDisplayText_number = function (rowidx) {
		...
		if (!mask && (v instanceof nexacro.Decimal || v instanceof nexacro.Number)) {
		...
		if (!isNaN(v)) {
			v = masknumber_api.removeMask(v, true);
		}

		var val = masknumber_api.normalizeValue(v, true, true);
		...
		var ninemask = this.__get_view_zeromask(mask);
		if (ninemask.nineidx > -1) {
			var org_val = this._org_val;
			if ((org_val == undefined || org_val.length <= 0) && ninemask.decimal <= 0) {
				num = "";
				for (var i = 0; i < ninemask.integer; i++) {
					num += "0";
				}
			}
			else if (org_val == "0" && ninemask.decimal == 0 && ninemask.integer == 0) {
				num = "0";
			}
		}
	...
	_pGridCellInfo._getDisplayText_date = function (rowidx, colType) {
		...
		if (v.constructor == nexacro.Date) {
			date = v;
		}
		else {
			var strVal = v.toString();

			for (var i = 0, str_len = strVal.length; i < str_len; i++) {
				if (strVal.charAt(i) != " ") {
					null_test = 1;
					break;
				}
			...
			if (null_test == 1) {
				...
				if (colType == 0) {
					...
					if (mask && mask.length > 0 && mask != "SHORTDATE" && mask != "LONGDATE") {
						timeonly = true;
						var controlcalendar = this.grid.controlcalendar;
						for (var i = 0, l = mask.length; i < l; i++) {
							if (controlcalendar._isDateMaskString(mask.charAt(i))) {
								timeonly = false;
								break;
							}
						}
					...
					if (colType == 1) {
						if (timeonly && strVal.length > 9) {
							timeonly = false;
						}
						v._timecheck = true;
					}

					date = this.__parseDate(strVal, colType, rowidx, timeonly);
				}
			}
			else {
				if (colType == 1) {
		...
		if (mask == "SHORTDATE" || mask == "LONGDATE") {
			...
			if (yyyy == 0) {
				...
				var str_yyyy = yyyy + "";
				if (str_yyyy.length == 1) {
					yyyy = "000" + str_yyyy;
				}
				else if (str_yyyy.length == 2) {
					yyyy = "00" + str_yyyy;
				}
				else if (str_yyyy.length == 3) {
					yyyy = "0" + str_yyyy;
				}
			}
			...
			displaycalendarctrl._monthlistS = locale_info.month_names_short;

			var MMM = displaycalendarctrl._monthlistS[date.getMonth()];
	...
	_pGridCellInfo.__parseDate = function (v, dFlag, rowidx, timeonly) {
		var regexp;
		switch (dFlag) {
			case 0:
				regexp = /(\d{6})/;
		...
		if (regexp.test(v) == false) {
			var mask = this._getAttrValue(this.mask, rowidx);
			if (mask && mask.length) {
				var tempcal = new nexacro.CalendarCtrl("tempcal", "absolute", 0, 0, 0, 0, null, null, this);
				tempcal.set_dateformat(mask);
				tempcal.set_editformat(mask);
	...
	_pGridCellInfo._getDisplayText_combo = function (rowidx) {
		...
		if (combodataset && combodataset.length && combocodecol && combocodecol.length && combodatacol && combodatacol.length) {
			...
			if (ds) {
				text = ds._lookupRaw(combocodecol, v, combodatacol);
			}
			...
			if (attr._bindtype == 1) {
				...
				if ((typeof exprfn) == "function") {
					if (cellinfo) {
						...
						var rowpos = (this.bandtype == "body") ? rowidx : grid._rowposition;
						return exprfn.call(cellinfo, rowidx, rowpos, grid, dataset);
					}
					else {
	...
	_pGridFormat._findDataset = function (id) {
		...
		var ds = this.grid._findDataset(id);
	...
	_pGridFormat._on_updateInnerDs = function (obj, e) {
		if (this.grid) {
			this.grid._on_useInnerDsCells(obj, e);
		}
	};
	...
	_pGridFormat.destroy = function () {
		...
		for (var i = 0, n = this._innerdatasets_name.length; i < n; i++) {
	...
	_pGridFormat._clearCellStyleCache = function (band, cellidx) {
		...
		if (cells) {
			if (cellidx > 0) {
				...
				for (var i = 0, subcells_len = subcells.length; i < subcells_len; i++) {
	...
	_pGridFormat._insertCell = function (cells, cell, addidx, rowidx) {
		...
		if (scol) {
			...
			cell._area = scol._area;
			cells.splice(add != undefined ? add : cells.length, 0, cell);
			return cell;
		}
		return undefined;
	};
	...
	_pGridFormat._loadFromDOM = function (formatElem) {
		...
		for (i = 0; i < len; i++) {
			...
			for (j = 0; j < cnt; j++) {
				...
				attrval = cellElem.getAttribute("imagestretch");
				if (attrval) {
					cellobj.set_imagestretch(attrval);
				}
	...
	_pGridFormat._sortCellIdx = function (cells, rows) {
		...
		var newcells = [];
		var cellsLen = cells.length;
		var rowsLen = rows.length, colsLen = this._cols.length;
		var cellidx = 0, colidx = 0, rowidx = 0;

		var idx = 0;
		while (cells.length) {
			if (cells[idx]._row == rowidx && cells[idx]._col == colidx) {
				cells[idx]._col = colidx++;
				cells[idx]._cellidx = cellidx++;
				newcells.push(cells.splice(idx, 1)[0]);
	...
	_pGridFormat._insertContentsCol = function (bandtype, bandobj, bandcells, bandrows, addfunc, insertidx) {
		...
		if (mergecells.length > 0) {
			for (var i = 0, n = mergecells.length; i < n; i++) {
		...
		for (var i = 0; i < rowsize; i++) {
			for (var j = 0, n = mergecells.length; j < n; j++) {
				...
				if (rowspan == 1) {
					...
					for (var j = 0; j < subcells.length; j++) {
						...
						if (subcell._row + cells[i]._row == row) {
							delete subcell;
							subcells.splice(j, 1);
							for (var k = j, subcells_n = subcells.length; k < subcells_n; k++) {
								subcells[k]._cellidx--;
							}
							j--;
						}
	...
	_pGridFormat.deleteContentsRow = function (strBand, nSubRowIndex, bBandIndex) {
		...
		if (retn != -1) {
			if (this.grid.getFormatRowCount() == 0) {
				var cols = this._cols;
	...
	_pGridFormat._insertContentsRow = function (bandtype, bandobj, bandcells, bandrows, rowfunc, addfunc, insertidx) {
		...
		if (mergecells.length > 0) {
			...
			for (var i = 0, mergecells_len = mergecells.length; i < mergecells_len; i++) {
				mergecell = mergecells[i];
			...
			for (var i = 0; i < cells.length; i++) {
				...
				if (cells[i]._col == col) {
					...
					if (colspan == 1) {
						delete cells[i];
						cells.splice(i, 1);
						for (var j = i, cells_n = cells.length; j < cells_n; j++) {
							cells[j]._cellidx--;
						}
						i--;
					}
	...
	_pGridFormat.deleteContentsCol = function (strBand, nColIndex, bBandIndex) {
		...
		if (retn != -1) {
			if (this.grid.getFormatColCount() == 0) {
				if (this._headrows) {
	...
	_pGridFormat.setFormatColProperty = function (nColIdx, strPropID, nValue) {
		if (this._cols) {
			if (strPropID == "size") {
				...
				for (var i = 0, cols_len = this._cols.length; i < cols_len; i++) {
					col = _cols[i];
					if (val == "left") {
