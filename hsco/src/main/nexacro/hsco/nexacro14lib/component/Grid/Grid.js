...
if (!nexacro.Grid) {
	...
	nexacro.GridEditCharEventInfo = function (obj, id, cell, chartext, col, pivotindex, posttext, pretext, row, subrow) {
	...
	var _pGridEditCharEventInfo = nexacro._createPrototype(nexacro.Event, nexacro.GridEditCharEventInfo);
	nexacro.GridEditCharEventInfo.prototype = _pGridEditCharEventInfo;
	_pGridEditCharEventInfo._type_name = "GridEditCharEventInfo";

	_pGridEditCharEventInfo.set_chartext = function (v) {
		this.chartext = v;
	};

	delete _pGridEditCharEventInfo;
	...
	nexacro.GridCell._default_linespace = nexacro._getCachedStyleObj("linespace", "0");
	...
	_pGridCell.on_apply_custom_pseudo = function (pseudo) {
		...
		if (curstyle.linespace != linespace) {
		...
		if (subComp && this._ReasonRefresh != 1) {
			...
			var cellinfo_style = cellinfo.style;
			var onlycontents = true;

			if (cellinfo_style.controlbackground && cellinfo_style.controlbackground._bindtype > 0) {
				onlycontents = false;
			}
			else if (cellinfo_style.controlborder && cellinfo_style.controlborder._bindtype > 0) {
				onlycontents = false;
			}
			else if (cellinfo_style.controlbordertype && cellinfo_style.controlbordertype._bindtype > 0) {
				onlycontents = false;
			}
			else if (cellinfo_style.controlgradation && cellinfo_style.controlgradation._bindtype > 0) {
				onlycontents = false;
			}

			subComp._updateAll(onlycontents);
		}
		...
		this._applyRowCellsStyle(pseudo);
	};

	_pGridCell._applyRowCellsStyle = function (pseudo, nocheck) {
		var grid = this._grid;

		if (this._is_created && (this._status_changing == true || nocheck)) {
	...
	_pGridCell.on_find_CurrentStyle_linespace = function (pseudo) {
		if (this._refobj) {
			var grid = this._grid, datarow = grid._getDataRow(this._rowidx);

			var linespace = this._refobj._query_pseudo_linespace(datarow, pseudo);
			return linespace ? linespace : nexacro.GridCell._default_linespace;
		}
		return null;
	};
	...
	_pGridCell.on_apply_style_linespace = function (linespace) {
		var text_elem = this._text_elem;

		if (text_elem) {
			text_elem.setElementLineSpace(linespace);
		}
	};
	...
	_pGridCell.on_change_status = function (status, pseudo) {
		this._status_changing = true;
		nexacro.Component.prototype.on_change_status.call(this, status, pseudo);
		this._status_changing = false;
	};

	_pGridCell._on_afterHideWaitComp = function (pseudo) {
		if (this._pseudo != pseudo) {
			if (pseudo == "mouseover") {
				var grid = this._grid;
				if (grid && (!grid._mouseovercell || grid._mouseovercell != this)) {
					return;
				}
			}
	...
	_pGridCell.on_apply_custom_setfocus = function (evt_name, self_flag) {
		var control_elem = this._control_element;

		var grid = this._grid;
		if (evt_name == "lbuttondown" || grid._track_mode == "areaselect") {
			grid._focus_proc = control_elem;
		}

		if (self_flag || nexacro._enableaccessibility || !this._no_dom_focus) {
			nexacro.Component.prototype.on_apply_custom_setfocus.call(this, evt_name);
		}


		if (nexacro._enableaccessibility) {
			grid.currentcell = this._cellidx;
			grid._currentBand = this._band.id;
		}
	};
	...
	_pGridCell._common_lbuttonup = function (changedtouchinfos, elem, canvasX, canvasY, from_elem) {
		...
		if (elem != from_elem) {
			...
			if (grid._selectborder_elements.length && upelem instanceof nexacro.GridCellControlElement) {
				this._is_real_upelem = undefined;
			}
			else {
				while (upelem) {
	...
	_pGridCell._on_touch_lbuttondown = function (elem, button, alt_key, ctrl_key, shift_key, canvasX, canvasY, screenX, screenY, event_bubbles, fire_comp, refer_comp, isTap) {
		var grid = this._grid;
		if (application.enabletouchevent == false && isTap == false && nexacro.isTouchInteraction && grid.selectscrollmode == "select") {
			grid._common_fire_user_lbuttondown(this, ctrl_key, shift_key, canvasX, canvasY, fire_comp, refer_comp);
		}
		return nexacro.Component.prototype._on_touch_lbuttondown.call(this, elem, button, alt_key, ctrl_key, shift_key, canvasX, canvasY, screenX, screenY, event_bubbles, fire_comp, refer_comp, isTap);
	};
	...
	_pGridCell._common_mouseenter = function (from_comp) {
		...
		if (from_comp != this) {
			...
			if (this._isEnable() && (nexacro.Browser == "Edge" || nexacro.Browser == "IE")) {
							...
							if (editor) {
								if (upelem instanceof nexacro.InputElement) {
									...
									if (editor.getCaretPos) {
										editor._edit_base_api._setCaret(upelem);
	...
	_pGridCell._getClickItem = function (halign, valign, canvasX, canvasY, text, comp) {
		...
		var clickitem = "";
		var size = nexacro._getTextSize2(comp.currentstyle.letterspace, text, comp.currentstyle.font, null, null, null, this._rtldirection);
		...
		if (halign == "left") {
			if (clientXY[0] >= paddingleft && clientXY[0] <= (size[0] + paddingleft)) {
				include = true;
			}
		}
		else if (halign == "center") {
			var l = (comp._client_width - size[0]) / 2 + paddingleft;
			var r = l + size[0];
			...
			if (clientXY[0] >= comp._client_width - size[0] + paddingleft) {
				include = true;
			}
		}
		...
		if (include) {
			if (valign == "top") {
				if (clientXY[1] >= paddingtop && clientXY[1] <= (size[1] + paddingtop)) {
					clickitem = "text";
				}
			}
			else if (valign == "middle") {
				var t = (comp._client_height - size[1]) / 2 + paddingtop;
				var b = t + size[1];
				...
				if (clientXY[1] >= comp._client_height - size[1] + paddingtop) {
					clickitem = "text";
				}
			}
		}
	...
	_pGridCell._on_dblclick = function (elem, button, alt_key, ctrl_key, shift_key, canvasX, canvasY, screenX, screenY) {
		if (!this._hideInner) {
			if (elem instanceof nexacro.ControlElement) {
				this._clickitem = "border";
			}
		...
		var retn = nexacro.Component.prototype._on_dblclick.call(this, elem, button, alt_key, ctrl_key, shift_key, canvasX, canvasY, screenX, screenY);
		this._clickitem = "";
		return retn;
	};
	...
	_pGridCell._applyMouseLeaveEvent = function () {
		if (this._is_created && this._pseudo == "mouseover") {
			var row = this.parent;
			var grid = this._grid;

			if (grid._isSelectRowType()) {
				row._updateAll("normal");
			}
			else {
				this._updateAll("normal");
			}
		}
	};

	_pGridCell._applyMouseOverEvent = function () {
		if (this._is_created && this._pseudo == "normal") {
			var row = this.parent;
			var grid = this._grid;
	...
	_pGridCell._createDisplayer = function (displayType) {
		...
		switch (displayType) {
			...
			case "tree":
				...
				grid._setTreeCellinfo(cellinfo);
				break;
			case "combo":
	...
	_pGridCell._updateAll = function (pseudo, onlycontents) {
		...
		if (control_elem) {
			...
			if (grid._currentCellRow >= 0) {
				...
				if (textelem) {
					var cellidx = this.parentcell ? this.parentcell._cellidx : this._cellidx;
					if (grid._currentCellCell == cellidx && grid._currentCellRow == datarow) {
				...
				if (subComp) {
					...
					if (grid._currentCellCell == cellidx && grid._currentCellRow == datarow) {
						...
						subComp.set_visible(true);
					}

					if (grid._isSelectRowType()) {
						subComp.on_apply_pseudo();
					}
				}
			}
			...
			if (grid._ReasonRefresh) {
				this._status = grid._ReasonRefresh._status;
			}
	...
	_pGridCell._getResultPseudo = function (status, pseudo) {
		...
		if ((grid._isFakeCell(datarow) && pseudo != "disabled") || nexacro._cur_track_info
			 || (nexacro.isTouchInteraction && pseudo == "mouseover")) {
			pseudo = "normal";
		}
		else {
			...
			if (w < bandrc.width) {
				if (l < 0) {
					if (hscroll._isEnable()) {
						hscroll._set_pixelpos(hscroll._pos + l);
					}
				}
				else if (r > bandrc.width) {
					var gap = r - bandrc.width;
					if (hscroll._isEnable()) {
						hscroll._set_pixelpos(hscroll._pos + gap);
					}
				}
			}
		}
	...
	_pGridCell._showEditor = function (flag, focus, show) {
		...
		grid._currentCellCol = cellinfo._col;
		grid._currentCellSubRow = cellinfo._row;
		...
		if (!flag) {
			...
			editComp = grid._currentCellEditor;
			if (editComp) {
	...
	_pGridCell._hideEditor = function () {
		...
		var grid = this._grid;
		grid._currentCellCell = -1;
		grid._currentCellRow = -1;
		grid._currentCellCol = -1;
		grid._currentCellSubRow = -1;

		if (this._subComp) {
			var subcomp = this._subComp;

			if (subcomp._displaymode && !subcomp._is_dblclick) {
				grid._changeDisplayer = true;
				this._updateDisplayer();
				this._updateAll();
				grid._changeDisplayer = false;
			}
			else {
				if (nexacro.Browser == "Runtime" && subcomp.setCaretPos) {
					subcomp.setCaretPos(0);
				}
	...
	_pGridCell.on_apply_prop_rtldirection = function () {
		...
		if (this._subComp) {
			this._subComp._setRtlDirection(this._rtldirection);
		}
	};
	delete _pGridCell;
	...
	_pGridExpand.on_find_CurrentStyle_background = function (pseudo) {
		if (this._is_push && this._is_pushed_area) {
			var background = this._find_pseudo_obj("background", "pushed", "background", true);
		}
		else {
			var background = this._find_pseudo_obj("background", pseudo, "background", true);
		}
		return background;
	};
	...
	_pGridExpand.on_find_CurrentStyle_cursor = function (pseudo) {
		var v = this._find_pseudo_obj("cursor", pseudo, "cursor", true);
		if (!v) {
			v = this._cellobj.on_find_CurrentStyle_cursor(this._cellobj._pseudo);
		}
		return v;
	};
	...
	_pGridButton.on_find_CurrentStyle_accessibility = function (pseudo) {
		...
		if (cellobj) {
			...
			if (!accessibility) {
				accessibility = cellobj.on_find_CurrentStyle_accessibility(pseudo);
			}
	...
	_pGridButton.on_find_CurrentStyle_cursor = function (pseudo) {
		...
		if (cellobj) {
			var cursor = this._find_pseudo_obj("cursor", pseudo) || cellobj.on_find_CurrentStyle_cursor(pseudo);
	...
	_pGridButton._on_click = function (elem, button, alt_key, ctrl_key, shift_key, canvasX, canvasY, screenX, screenY) {
		...
		if (this._is_dblclick || (this._displaymode && (this._grid.selectchangetype == "up"))) {
			visible = true;
		}

		if (this._cellobj._is_clickproc) {
			return;
		}
	...
	_pGridButton._updateAll = function (onlycontents) {
		this.on_apply_pseudo(this._cellobj._pseudo, onlycontents);
		this._setProperty();
	};
	...
	_pGridBar._find_pseudo_obj = function (styleProp, pseudo, returnType) {
		...
		if (cellobj) {
			...
			if (cellobj && this._grid._isFakeCell(datarow) && pseudo != "disabled") {
				pseudo = "normal";
			}

			if (styleProp == "barcolor" || styleProp == "bardirection" || styleProp == "bargradation") {
				var barProp = cellinfo.style[styleProp];
				if (barProp && styleProp == "barcolor") {
					return cellinfo._getAttrValue(barProp, datarow);
				}
				controlProp = "";
			}
	...
	_pGridBar._updateAll = function (onlycontents) {
		if (this.getElement()) {
			this.on_apply_pseudo(this._cellobj._pseudo, onlycontents);
			this._setProperty();
		}
	...
	_pGridEdit.on_fire_onchar = function (event_info, textEvt) {
		...
		if (!textEvt) {
			...
			nexacro.EditCtrl.prototype.on_fire_onchar.call(this, event_info, obj);
			textEvt = obj.evt;
		}
		else {
			nexacro.EditCtrl.prototype.on_fire_onchar.call(this, event_info, textEvt);
		}

		return this._grid.on_fire_onchar(this, event_info.chartext, event_info.prechareventtext, event_info.posttext, textEvt);
	};

	_pGridEdit.on_fire_ontextchange = function (event_info, textEvt) {
		if (this._absolutelyValue == true) {
			return true;
		}
		...
		if (!textEvt) {
			...
			nexacro.EditCtrl.prototype.on_fire_ontextchange.call(this, event_info, obj);
			textEvt = obj.evt;
		}
		else {
			nexacro.EditCtrl.prototype.on_fire_ontextchange.call(this, event_info, textEvt);
		}

		return this._grid.on_fire_ontextchange(this, event_info.chartext, event_info.pretext, event_info.posttext, event_info.preimetext, event_info.postimetext, textEvt);
	};
	...
	_pGridEdit.on_fire_user_onkeydown = function (keycode, alt_key, ctrl_key, shift_key, fire_comp, refer_comp) {
		if (nexacro.Browser == "Edge" && keycode == nexacro.Event.KEY_ENTER) {
			nexacro.EditCtrl.prototype._on_input_keydown.call(this, this._input_element, keycode, alt_key, ctrl_key, shift_key);
			nexacro.EditCtrl.prototype._on_input_keypress.call(this, this._input_element, keycode, keycode, alt_key, ctrl_key, shift_key);
		}

		return nexacro.EditCtrl.prototype.on_fire_user_onkeydown.call(this, keycode, alt_key, ctrl_key, shift_key, fire_comp, refer_comp);
	};
	...
	_pGridEdit._updateAll = function (onlycontents) {
		if (this.getElement()) {
			this.on_apply_pseudo(this._cellobj._pseudo, onlycontents);
			this._setProperty();
		}
	...
	_pGridEdit._setProperty = function () {
		...
		this.set_usesoftkeyboard(grid.usesoftkeyboard);
	};
	...
	_pGridTextArea.on_fire_onchar = function (event_info, textEvt) {
		if (this._absolutelyValue == true) {
			return true;
		}
		...
		if (!textEvt) {
			...
			nexacro.TextAreaCtrl.prototype.on_fire_onchar.call(this, event_info, obj);
			textEvt = obj.evt;
		}
		else {
			nexacro.TextAreaCtrl.prototype.on_fire_onchar.call(this, event_info, textEvt);
		}

		return this._grid.on_fire_onchar(this, event_info.chartext, event_info.prechareventtext, event_info.posttext, textEvt);
	};

	_pGridTextArea.on_fire_ontextchange = function (event_info, textEvt) {
		if (this._absolutelyValue == true) {
			return true;
		}
		...
		if (!textEvt) {
			...
			nexacro.TextAreaCtrl.prototype.on_fire_ontextchange.call(this, event_info, obj);
			textEvt = obj.evt;
		}
		else {
			nexacro.TextAreaCtrl.prototype.on_fire_ontextchange.call(this, event_info, textEvt);
		}
	...
	_pGridTextArea.on_fire_user_onkeydown = function (keycode, alt_key, ctrl_key, shift_key, fire_comp, refer_comp) {
		if (nexacro.Browser == "Edge" && keycode == nexacro.Event.KEY_ENTER) {
			nexacro.TextAreaCtrl.prototype._on_input_keydown.call(this, this._input_element, keycode, alt_key, ctrl_key, shift_key);
			nexacro.TextAreaCtrl.prototype._on_input_keypress.call(this, this._input_element, keycode, keycode, alt_key, ctrl_key, shift_key);
		}
		return nexacro.TextAreaCtrl.prototype.on_fire_user_onkeydown.call(this, keycode, alt_key, ctrl_key, shift_key, fire_comp, refer_comp);
	};
	...
	_pGridTextArea._updateAll = function (onlycontents) {
		if (this.getElement()) {
			this.on_apply_pseudo(this._cellobj._pseudo, onlycontents);
			this._setProperty();
		}
	...
	_pGridMaskEdit._on_fire_changeEventSet = function (pretext, prevalue, posttext, postvalue) {
		var retn = nexacro.MaskEditCtrl.prototype._on_fire_changeEventSet.call(this, pretext, prevalue, posttext, postvalue);
		if (retn) {
	...
	_pGridMaskEdit.on_fire_onchar = function (event_info) {
		if (this._absolutelyValue == true) {
			return true;
		}
		...
		nexacro.MaskEditCtrl.prototype.on_fire_onchar.call(this, event_info, obj);
		return this._grid.on_fire_onchar(this, event_info.chartext, event_info.prechareventtext, event_info.posttext, obj.evt);
	};
	...
	_pGridMaskEdit.on_fire_ontextchange = function (event_info) {
		...
		nexacro.MaskEditCtrl.prototype.on_fire_ontextchange.call(this, event_info, obj);
		return this._grid.on_fire_ontextchange(this, event_info.chartext, event_info.pretext, event_info.posttext, event_info.preimetext, event_info.postimetext, obj.evt);
	};
	...
	_pGridMaskEdit._updateAll = function (onlycontents) {
		if (this.getElement()) {
			this._setProperty();
			this.on_apply_pseudo(this._cellobj._pseudo, onlycontents);
		}
	...
	_pGridMaskEdit._setDataset = function (async, row) {
		...
		if (cellinfo.text._bindtype == 1) {
			...
			if (this._textchanging || (this._is_focusing && (focus_text != cur_text || focus_val != cur_val))) {
				grid._is_async_recreate = async;
				grid._dsEventOccured = true;
				...
				cellinfo.grid._binddataset._setColumn(datarow, cellinfo.text._bindexpr, cur_val, fail);
	...
	_pGridCalendar.on_find_CurrentStyle_buttonsize = function (pseudo) {
		var retn = nexacro.CalendarCtrl.prototype.on_find_CurrentStyle_buttonsize.call(this, pseudo);
	...
	_pGridCalendar.on_find_CurrentStyle_align = function (pseudo, childctrl) {
		...
		if (cellobj) {
			...
			var align = this._find_pseudo_obj("align", pseudo, "align", this.calendaredit);
			return align;
		}
		return null;
	};
	...
	_pGridCalendar.on_find_CurrentStyle_font = function (pseudo, childctrl) {
		...
		if (cellobj) {
			var font = this._find_pseudo_obj("font", pseudo, "font", this.calendaredit);
	...
	_pGridCalendar.on_find_CurrentStyle_letterspace = function (pseudo, childctrl) {
		...
		if (cellobj) {
			var letterspace = this._find_pseudo_obj("letterspace", pseudo, "letterspace", this.calendaredit);
	...
	_pGridCalendar.on_find_CurrentStyle_color = function (pseudo, childctrl) {
		...
		if (cellobj) {
			var color = this._find_pseudo_obj("color", pseudo, "color", this.calendaredit);
	...
	_pGridCalendar.on_notify_ondropdown = function (obj, e) {
		if (!this._displaymode) {
			var grid = this._grid;
			var ret = nexacro.CalendarCtrl.prototype.on_notify_ondropdown.call(this, obj, e);
			if (this._isPopupVisible()) {
				grid._is_editor_keyaction = false;
			}
			else {
				grid._is_editor_keyaction = true;
			}
			return ret;
		}
	};
	...
	_pGridCalendar.on_notify_mobile_ondropdown = function (obj, e) {
		if (!this._displaymode) {
			var grid = this._grid;
			var ret = nexacro.CalendarCtrl.prototype.on_notify_mobile_ondropdown.call(this, obj, e);
			if (this._isPopupVisible()) {
				grid._is_editor_keyaction = false;
			}
			else {
	...
	_pGridCalendar.on_fire_onchar = function (obj, chartext, pretext, posttext) {
		nexacro.CalendarCtrl.prototype.on_fire_onchar.call(this, obj, chartext, pretext, posttext);
		var textEvt = new nexacro.CharEventInfo(this, "onchar", chartext, pretext, posttext);
		return this._grid.on_fire_onchar(obj, chartext, pretext, posttext, textEvt);
	};

	_pGridCalendar.on_notify_onlbuttondown = function (obj, e) {
		if (!this._displaymode) {
			return nexacro.CalendarCtrl.prototype.on_notify_onlbuttondown.call(this, obj, e);
		}
	};

	_pGridCalendar.on_fire_ontextchange = function (obj, chartext, pretext, posttext, preimetext, postimetext) {
		var textEvt = new nexacro.TextChangeEventInfo(this, "ontextchange", chartext, pretext, posttext, preimetext, postimetext);
		return this._grid.on_fire_ontextchange(this, chartext, pretext, posttext, preimetext, postimetext, textEvt);
	};
	...
	_pGridCalendar.on_fire_onchanged = function (obj, pre_text, pre_value, post_text, post_value) {
		if (!obj._displaymode) {
			if (this._grid.autoupdatetype == "dateselect" || this._grid.autoupdatetype == "itemselect") {
				this._setDataset(true, undefined, false, pre_value);
			}
	...
	_pGridCalendar._updateAll = function (onlycontents) {
		if (this.getElement()) {
			this.on_apply_pseudo(this._cellobj._pseudo, onlycontents);
			this._setProperty();
		}
		...
		if (calendardisplaynulltype != "default" && (nexacro._isNull(org_v) || org_v === "")) {
			if (this._displaymode) {
				if (calendardisplaynulltype == "nulltext") {
					var calendardisplaynulltext = cellinfo._getAttrValue(cellinfo.calendardisplaynulltext, datarow);
					this.set_dateformat(calendardisplaynulltext);
				}
				else {
	...
	_pGridCalendar._setDataset = function (async, row, fire, oldvalue) {
		var cellobj = this._cellobj;
		var cellinfo = this._cellinfo;
		var grid = this._grid;
		var datarow = grid._getDataRow(cellobj._rowidx);
	...
	_pGridCombo.on_find_CurrentStyle_align = function (pseudo, childctrl) {
		...
		if (cellobj) {
			...
			var align = this._find_pseudo_obj("align", pseudo, "align", this.comboedit);
			return align;
		}
		return null;
	};
	...
	_pGridCombo.on_find_CurrentStyle_font = function (pseudo, childctrl) {
		...
		if (cellobj) {
			var font = this._find_pseudo_obj("font", pseudo, "font", this.comboedit);
	...
	_pGridCombo.on_find_CurrentStyle_letterspace = function (pseudo, childctrl) {
		...
		if (cellobj) {
			var letterspace = this._find_pseudo_obj("letterspace", pseudo, "letterspace", this.comboedit);
	...
	_pGridCombo.on_find_CurrentStyle_color = function (pseudo, childctrl) {
		...
		if (cellobj) {
			if (this._displaymode && pseudo == "normal") {
				pseudo = "readonly";
			}

			var color = this._find_pseudo_obj("color", pseudo, "color", this.comboedit);
	...
	_pGridCombo.on_fire_ontextchange = function (obj, e) {
		...
		return this._grid.on_fire_ontextchange(this, e.chartext, e.pretext, e.posttext, e.preimetext, e.postimetext, e);
	};
	...
	_pGridCombo._updateAll = function (onlycontents) {
		if (this.getElement()) {
			this.on_apply_pseudo(this._cellobj._pseudo, onlycontents);
			this._setProperty();
		}
	...
	_pGridCellCheckbox._updateAll = function (onlycontents) {
		if (this.getElement()) {
			this._setProperty();
			this.on_apply_pseudo(this._cellobj._pseudo, onlycontents);
	...
	_pGridCellCheckbox._setProperty = function () {
		...
		cellobj._setAccessibilityStatChecked(this.checked);
	};
	...
	_pGridImage.on_apply_imagestretch = function () {
		var cellobj = this._cellobj;
		var img_elem = this._img_elem;
		...
		if (cellobj && img_elem) {
			...
			var stretch = cellinfo._getAttrValue(cellinfo.imagestretch, datarow);

			if (stretch != "none") {
				var cell_width = cellobj._client_width;
				var cell_height = cellobj._client_height;

				if (stretch == "fit") {
					img_elem.image_width = this.imagewidth = cell_width;
					img_elem.image_height = this.imageheight = cell_height;
				}
				else if (stretch == "fixaspectratio") {
					var width = 0, height = 0;

					var image_width = this.imagewidth;
					var image_height = this.imageheight;

					var w_ratio = cell_width / image_width;
					var h_ratio = cell_height / image_height;

					if (w_ratio <= h_ratio) {
						width = cell_width;
						height = Math.floor(image_height * w_ratio);
					}
					else {
						width = Math.floor(image_width * h_ratio);
						height = cell_height;
					}

					img_elem.image_width = this.imagewidth = width;
					img_elem.image_height = this.imageheight = height;
				}

				img_elem.setElementSize(this.imagewidth, this.imageheight);
			}

			cell_width = null, cell_height = null;
			cellinfo = null, grid = null;
			stretch = null;
		}
	...
	_pGridImage._updateAll = function (onlycontents) {
		this.on_apply_pseudo(this._cellobj._pseudo, onlycontents);
		this._setProperty();
	};
	...
	_pGridImage._load_image = function (image) {
		...
		if (val && elem) {
			...
			if (size) {
				...
				_img_elem.setElementVisible(true);
			}
	...
	_pGridTree._btnimg_size = {
		width : 0, 
		height : 0
	};

	_pGridTree.on_change_status = function (status, pseudo) {
		this._status_changing = true;
		nexacro.Component.prototype.on_change_status.call(this, status, pseudo);
		this._status_changing = false;
	};
	...
	_pGridTree.on_apply_custom_pseudo = function (pseudo) {
		...
		if (this._status_changing) {
			this._cellobj._applyRowCellsStyle(pseudo, true);
		}
	};


	_pGridTree.on_find_CurrentStyle_background = nexacro._emptyFn;
	...
	_pGridTree._createCheckbox = function (bCreateOnly) {
		if (!this._grid.treeusecheckbox) {
			return;
		}
	...
	_pGridTree._createLines = function (bCreateOnly) {
	...
	_pGridTree._createImage = function () {
		if (!this._grid.treeuseimage) {
			return;
		}
	...
	_pGridTree._createButton = function () {
		if (!this._btn_elem) {
			var control_elem = this.getElement();
	...
	_pGridTree._on_lbuttonup = function (elem, button, alt_key, ctrl_key, shift_key, canvasX, canvasY, screenX, screenY, event_bubbles, fire_comp, refer_comp, from_elem) {
		if (!this._is_alive) {
			return;
		}

		var grid = this._grid;
		if (!this._isEditTypeTree() && grid.autoenter == "select") {
			var cellobj = this._cellobj;
			cellobj._common_lbuttonup(null, elem, canvasX, canvasY, from_elem);
		}
	...
	_pGridTree._on_click = function (elem, button, alt_key, ctrl_key, shift_key, canvasX, canvasY, screenX, screenY) {
		if (!this._is_alive || this._grid._treetarget) {
			this._grid._treetarget = null;
			return;
		}
		...
		if (this.visible && this._isEnable() && this.enableevent) {
			...
			else if ((elem instanceof nexacro.ImageElement) || (elem instanceof nexacro.AlignImageElement)) {
	...
	_pGridTree._createLeftLine = function (rowidx, linetype) {
		...
		while (grid._rootlevel < parentlevel) {
			...
			if (bExistNextParentNode) {
				...
				if (!ctrl) {
					ctrl = new nexacro.GridControlLine("controlleftline", 0, 0, 1, parentheight, this.parent);
	...
	_pGridTree.on_apply_prop_rtldirection = function () {
		nexacro.Component.prototype.on_apply_prop_rtldirection.call(this);
		var _rtldirection = this._rtldirection;

		if (this._chk_ctrl) {
			this._chk_ctrl._setRtlDirection(_rtldirection);
		}
		if (this._rightline_ctrl) {
			this._rightline_ctrl._setRtlDirection(_rtldirection);
		}
		if (this._upline_ctrl) {
			this._upline_ctrl._setRtlDirection(_rtldirection);
		}
		if (this._downline_ctrl) {
			this._downline_ctrl._setRtlDirection(_rtldirection);
		}
	...
	_pGridTree._adjustSubCompAlign = function (level) {
		...
		var lineleft = offset < 0 ? defaultsize : offset;
		...
		if (this._btn_elem) {
			var buttonWidth = this._btnimg_size ? this._btnimg_size.width : this._btn_elem.width;

			if (buttonWidth <= 0) {
				buttonWidth = defaultsize;
			}

			var buttonHeight = this._btnimg_size ? this._btnimg_size.height : this._btn_elem.height;

			if (buttonHeight <= 0) {
				buttonHeight = defaultsize;
			}

			var tmp_buttonLeft = 0;
			var buttonLeft = offset - Math.round(buttonWidth / 2);
			if (buttonLeft < 0) {
				tmp_buttonLeft = buttonLeft;
				buttonLeft = offset - Math.round(defaultsize / 2);
				this._btn_elem.setElementSize(buttonWidth, buttonHeight);
			}
			...
			linewidth = buttonWidth + tmp_buttonLeft;
			this._btn_elem.setElementPosition(buttonLeft, buttonTop, true);
		}
		else {
		...
		if (this._img_elem && this._img_elem.visible) {
			...
			this._img_elem.setElementPosition(imageLeft, imageTop, true);
		...
		if (this._text_elem) {
			...
			this._text_elem.setElementPosition(textLeft, textTop, true);
	...
	_pGridRow.on_apply_custom_setfocus = function (evt_name, self_flag) {
		if (self_flag) {
			nexacro.Component.prototype.on_apply_custom_setfocus.call(this, evt_name);
		}
	};
	...
	_pGridRow._resetCellsSize = function (format, startcol) {
		...
		for (var i = 0; i < cells_len; i++) {
			...
			if (!cols[cellinfo._col]) {
				continue;
			}

			left = Math.floor(cols[cellinfo._col].left);
			top = _row_tops[cellinfo._row];
			width = Math.floor(cols[cellinfo._col + cellinfo._colspan - 1].right - left);
			height = _row_bottoms[cellinfo._row + cellinfo._rowspan - 1] - top;

			cell.move(left, top, width, height);
			expand_ctrl = cell._expandCtrl;
			if (expand_ctrl) {
				left = width - cell._expand_width;
				top = expand_ctrl.top;
				width = cell._expand_width;
			...
			for (var j = 0; j < subcells_len; j++) {
				...
				left = Math.floor(cols[subcol].left);
				top = _row_tops[subrow];
				width = Math.floor(cols[subcol + subcellinfo._colspan - 1].right - left);
				height = _row_bottoms[subrow + subcellinfo._rowspan - 1] - top;

				left -= Math.ceil(cols[cellinfo._col].left);
				top -= _row_tops[cellinfo._row];
	...
	_pGridMatrixManager._addRow = function (top, height, rowidx, is_scrolling, is_fixed) {
		...
		row._fixed = !!is_fixed;
		row.createComponent();
		row._setRtlDirection(this._band._rtldirection);
		if (!is_scrolling) {
	...
	_pGridMatrixManager._adjustScrollRows = function (vpos, is_updatecontents) {
		...
		if (dir > 0) {
			...
			for (var i = 0, n = rows.length; i < n; i++) {
				if (!is_updatecontents && this._isShowScreenRow(rows[i], vpos, band._client_height) == true) {
					break;
				}
	...
	_pGridBand.on_find_CurrentStyle_selectborder = function (pseudo) {
		return this._find_pseudo_obj("selectborder", pseudo, "border");
	};
	...
	_pGridBand.on_change_containerRect = function (width, height) {
		...
		if (this._isBody) {
			...
			if (grid._needUpdateExtinner()) {
				grid._recreate_contents_all(false, false, false, true);
			}
			else {
				this._matrix._adjustRowsDisplay(true);
				this._matrix._adjustColsDisplay(bRtl);
	...
	_pGridBand.on_apply_custom_setfocus = function (evt_name, self_flag) {
	...
	_pGridBand._on_refresh_rows_physical = function (update_rows, create_rows, scrolling, no_update_supp) {
		...
		if (this._isBody) {
			...
			if (grid._is_created == true) {
				...
				var create_row = null, update_row = null;
				...
				for (var i = 0; i < update_rows_len; i++) {
					update_row = update_rows[i];

					if (update_row._is_alive) {
						if (grid._isSelectRowType()) {
							pseudo = update_row._pseudo;
						}
						update_row._updateAll(pseudo, undefined, onlycontents);
					}
				}

				if (grid._is_use_suppress) {
					var cellinfo = null, cells = null, cells_cnt = 0;
					var rowidx, datarow;
					...
					for (var j = 0; j < rows_len; j++) {
						...
						cells = rows[j]._cells;
						cells_cnt = cells ? cells.length : 0;

						for (var i = 0; i < cells_cnt; i++) {
							cellinfo = cells[i]._refobj;
							if (cellinfo && cellinfo.suppress != 0) {
								this._refreshRowCell(j, i, grid._isSelectedCell(i, datarow), cells[i]._pseudo);
							}
						}
					}
				}
	...
	_pGridBand._recreate_contents = function (recreate_colarea, init_scroll, scrolling, no_hide_edit, no_update_supp) {
		...
		var _hpos = (grid.hscrollbar) ? grid.hscrollbar._pos : 0, _vpos = (grid.vscrollbar) ? grid.vscrollbar._pos : 0, scrollwidth = format.bodyWidth, scrollheight = 0;
		...
		if (this._isBody) {
			...
			var rowsize = 0, band_scroll_sizes = [], band_sizes_cnt = 1, band_scroll_max = grid._div_max_height;

			for (var i = 0; i < rowcnt; i++) {
				rowsize = grid._getRowSize(i);
				scrollheight += rowsize;

				if (scrollheight - grid._fixedrow_height >= band_scroll_max * band_sizes_cnt) {
					band_scroll_sizes.push(scrollheight - rowsize);
					band_sizes_cnt++;
				}
			}
			...
			if (init_scroll || grid._isChangeBandSrollSizeList(band_scroll_sizes)) {
			...
			if ((!grid._headBand && !grid._summBand) || grid._hasTree) {
				grid._setScrollMaxSize(this._scrollWidth, this._scrollHeight, band_scroll_sizes);
			}
	...
	_pGridScrollBarCtrl._set_pixelpos = function (v, evt_kind) {
		var str = this._getScrollInfo(v, evt_kind);
		...
		if (grid._scrollpixel != "all" || is_notcheck) {
			if (this.direction == "vert") {
				...
				if (format) {
					...
					var leftcnt = format._getColFixCnt("left");

					col = v + leftcnt;

					for (var i = leftcnt; i < col; i++) {
						width += cols[i].size;
					}
					v = width;
				}
			...
			if (this.direction == "vert") {
				...
				if (format) {
					if (v == this._pos) {
						...
						var cols = format._cols;
						var leftcnt = format._getColFixCnt("left");
						var dec = (v < this._pos && !this._is_tracking);

						if (v > 0) {
							for (var i = leftcnt; i < cols.length; i++) {
								col = i;
								if (v <= width) {
									if (!is_max) {
										if (dec) {
										...
										renew = width <= this._max ? width : this._max;
									}
									break;
								}
								width += cols[col].size;
	...
	_pGridScrollBarCtrl._setScrollPos = function (si_pos) {
		...
		if (grid._scrollpixel != "all" && this.direction == "vert") {
	...
	nexacro.Grid = function (id, position, left, top, width, height, right, bottom, parent) {
		...
		this._sizetextarea = new nexacro.TextAreaCtrl("controltextarea", "absolute", 0, -10, 0, 1, null, null, this);
		this._sizetextarea.set_visible(false);
		...
		this._realrowsized_body = [];
		this._realrowsized_head = [];
		this._realrowsized_summ = [];
	};
	...
	_pGrid.on_create_contents = function () {
		...
		if (control_elem) {
			...
			this._sizetextarea.createComponent(true);
		}
	};
	...
	_pGrid.on_created_contents = function () {
		...
		if (this._create_selection != null && !this._isMultiSelect() && !this._isAreaSelect()) {
	...
	_pGrid.on_destroy_contents = function () {
		...
		this._destroySelectBorderElements();
		...
		this._selectborder_elements = null;
		this._recalcXY_info = null;
		this._fake_mergecell_arr = null;
		this._enable_redraw_history = null;
		this._autofitcol_rate = null;
		this._org_treeStates = null;
		this._realrowsized_body = null;
		this._realrowsized_head = null;
		this._realrowsized_summ = null;
	...
	_pGrid._createHScrollBar = function (scroll_size) {
		var hscrollbar = new nexacro.GridScrollBarCtrl("hscrollbar", "absolute", 0, 0, this._client_width, scroll_size, null, null, this);
	...
	_pGrid._applyAreaSizing = function (posobj, idx, is_tracking) {
		...
		if (cellobj && cellobj._control_element != this._focus_proc && kind === undefined) {
			this._focus_proc = cellobj._control_element;
		}
	};
		...
		if (!value) {
			...
			if (update == true) {
				if (this.autosizingtype == "col" || this.autosizingtype == "both") {
					this._recreate_contents_all(false, true, false);
				}
				else {
					this._recreate_contents_all(false, false, false, true);
				}
				...
				if (nSubRowIndex >= 0) {
					if (nSubRowIndex < rowsLen) {
						...
						if (oldsize != newsize) {
							...
							this._realrowsized_body[index] = true;
							change = true;
				...
				if (nSubRowIndex >= 0) {
					if (nSubRowIndex < rowsLen) {
						...
						if (oldsize != newsize) {
							...
							this._realrowsized_head[index] = true;
							change = true;
				...
				if (nSubRowIndex >= 0) {
					if (nSubRowIndex < rowsLen) {
						...
						if (oldsize != newsize) {
							_rowSizeListSub[index] = newsize;
							_rowSizeList[0] += (newsize - oldsize);
							this._realrowsized_summ[index] = true;
							change = true;
		...
		if (cellinfo) {
			...
			if (strPropID == "displaytype" && varValue == "tree") {
				...
				if (strBand == "head" && this._headAutoSize) {
					...
					this._refreshCell(strBand, nCellIdx, -1, true, true);
				}
			}
			...
			else if (strPropID == "linespace") {
				this._recreate_contents_all(true, true, false, true);
			}
			else {
				var is_onlycontents = false;
				if (strPropID == "displaytype" || strPropID == "editdisplay" || strPropID == "combodisplay"
					 || strPropID == "calendardisplay" || strPropID == "edittype" || strPropID == "imagestretch") {
					this._changeDisplayer = true;
				}
				else if (strPropID == "wordwrap") {
					is_onlycontents = true;
					if (varValue != "none") {
						if (strBand == "head") {
				...
				else if (strPropID == "tooltiptext") {
					is_onlycontents = true;
				}

				this._refreshCell(strBand, nCellIdx, -1, true, is_onlycontents);
	...
	_pGrid._adjustGridScrollRows_callback = function (no_ani) {
		...
		this._adjustOverlayElements(false, this._is_use_fakemerge, true);
		this._setSelectBorder();
	};
	...
	_pGrid.on_fire_user_ondragenter = function (src_comp, src_refer_comp, dragdata, userdata, button, alt_key, ctrl_key, shift_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp) {
		if (this._noFireDragFlag == true || (src_comp && src_comp.selectscrollmode && (src_comp.selectscrollmode == "scroll" || (nexacro._isMobile && nexacro._isMobile() && src_comp.selectscrollmode == "default")))) {
			return;
		}
	...
	_pGrid.on_fire_sys_ondragmove = function (src_comp, src_refer_comp, dragdata, userdata, button, alt_key, ctrl_key, shift_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp, xdeltavalue, ydeltavalue) {
		if (!this._is_drag_sameselect) {
			if (this._isAreaSelect()) {
				return this._areaselectMove(from_refer_comp, canvasX, canvasY);
			}
			else {
				return nexacro.Component.prototype.on_fire_sys_ondragmove.call(this, src_comp, src_refer_comp, dragdata, userdata, button, alt_key, ctrl_key, shift_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp, xdeltavalue, ydeltavalue);
			}
		}
		else {
	...
	_pGrid.on_fire_user_ondrag = function (button, alt_key, ctrl_key, shift_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp, self_refer_comp) {
		...
		if (this.ondrag && this.ondrag._has_handlers) {
			...
			evt = new nexacro.GridDragEventInfo(this, "ondrag", dragdata, userdata, this, self_refer_comp, from_comp, from_refer_comp, button, alt_key, ctrl_key, shift_key, screenX, screenY, canvasX, canvasY, clientX, clientY, cell, col, mergecell, mergecol, mergerow, pivotindex, row, subrow);
		}
		...
		else if (this.cellmovingtype != "none" || evt) {
			if (cellobj && cellobj._type_name == "GridCell" && cellobj._rowidx == -1) {
				if (this.cellmovingtype != "none") {
					...
					if (info0[1] == 1 && (info1 == null || info1[1] == 1) && (info2 == null || info2[1] == 1)) {
						if (nexacro.isTouchInteraction && application.enabletouchevent == false) {
							application._touch_manager._tap_blocked = true;
						}
				...
				if (nexacro._toBoolean(this.readonly) == false) {
					if (retn) {
						...
						if (cell && cell._hasEditor()) {
							...
							var editor = this._currentCellEditor;
							if (nexacro.Browser == "Gecko" && cell._curEditDisplay == "display") {
	...
	_pGrid._recalcXY = function (obj, canvasX, canvasY, need_recalcXY, from_refer_comp) {
		...
		if (from_refer_comp._type_name != "Grid") {
			if (from_refer_comp._type_name == "GridRow" && from_refer_comp._rowidx >= 0) {
				real_canvasY -= this._getScrollTop();
			}
	...
	_pGrid.on_fire_sys_onrbuttondown = function (button, alt_key, ctrl_key, shift_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp) {
		var ret = nexacro.Component.prototype.on_fire_sys_onrbuttondown.call(this, button, alt_key, ctrl_key, shift_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp);


		var cellobj = from_refer_comp;
		cellobj = this._findCellObj(cellobj);

		if (cellobj && cellobj instanceof nexacro.GridCell && this.autoenter == "select") {
			if (cellobj.parentcell) {
				var subcellobj = cellobj;
				cellobj = cellobj.parentcell;
			}

			if (cellobj && cellobj._hasEditor() && cellobj._is_Selected()) {
				this._beforebodycellpos = this._selectinfo.curcell;
				this._beforebodycolpos = this._selectinfo.curcol;
				this._beforebodyrowpos = this._selectinfo.curdsrow;
				this._beforebodysubrowpos = this._selectinfo.cursubrow;
				this._beforepvt = this._selectinfo.curpvt;
				...
				_win._postMessage(this, "showeditor", this._callbackPostmsg, [false, undefined, true]);
			}
			else {
				if (this._showEditing) {
					this._hideEditor();
	...
	_pGrid._getFakeMergeCellObjs = function (fake_mergecell) {
		...
		for (var i = 0, n = rows.length; i < n; i++) {
			...
			if (datarow >= srow && datarow <= erow) {
				...
				for (var j = 0, cells_len = cells.length; j < cells_len; j++) {
	...
	_pGrid._setFakeMerge = function (scol, srow, ssubrow, ecol, erow, esubrow, norelease) {
		...
		for (var i = 0, cellinfos_len = cellinfos.length; i < cellinfos_len; i++) {
			cell = cellinfos[i];
			col1 = cell._col;
		...
		if (fail_idxs.length == 0) {
			for (var i = 0, n = target_cellinfos.length; i < n; i++) {
			...
			for (var i = 0, n = fail_idxs.length; i < n; i++) {
				var idx = fail_idxs[i];
				...
				for (var j = 0, release_cellinfos_len = release_cellinfos.length; j < release_cellinfos_len; j++) {
				...
				for (var j = 0, cellinfos_len = target_cellinfos.length; j < cellinfos_len; j++) {
	...
	_pGrid._on_last_lbuttonup = function (down_act) {
		...
		if (args != null) {
			...
			if (!this._showEditing && this.autoenter == "select") {
				this._showEditor();
			}
		}
	...
	_pGrid._on_last_keyup = function (down_act) {
		...
		if (this._after_recreate) {
			...
			var display_row = this._dsRowToDispRow(this._rowposition);
			this._jumpCurrentRow(display_row);
		}
	};
	...
	_pGrid._areaselectMove = function (from_refer_comp, canvasX, canvasY) {
		if (this._is_drag_selectstart && !this._showEditing) {
			...
			if (cellobj && cellobj._type_name == "GridCell") {
				...
				if (!nexacro._enableaccessibility) {
					cellobj._no_dom_focus = true;
					cellobj._setFocus(false);
					cellobj._no_dom_focus = false;
				}
	...
	_pGrid._setSelectedInfo = function (cell, col, datarow, subrow, pvt, areainfo) {
		if (cell !== null) {
			this._selectinfo.curcell = cell;
		}
		if (col !== null) {
			this._selectinfo.curcol = col;
		}
		if (datarow !== null) {
			this._selectinfo.curdsrow = datarow;
			this._selectinfo.currow = (datarow < 0) ? datarow : this._getTreeRowPosition(datarow);
		}

		if (subrow !== null) {
			this._selectinfo.cursubrow = subrow;
		}
		if (pvt !== null) {
			this._selectinfo.curpvt = pvt;
		}
		...
		if (this._currentBand == "body") {
			...
			if (datarow !== null) {
				this._currentDSrow = datarow;
				this.currentrow = (datarow < 0) ? datarow : this._getTreeRowPosition(datarow);
			}

			if (subrow !== null) {
				this.currentsubrow = subrow;
			}
			if (pvt !== null) {
				this.currentpivot = pvt;
			}
		}
	...
	_pGrid._on_grid_lbuttondown = function (cellobj, band, ctrlkey, shiftkey, no_select) {
		...
		if (!retn) {
			if (!this._isIncludeSelectpos(afterCell, afterRow)) {
				this._is_drag_selectstart = false;
			}
			this._is_drag_sameselect = true;
		}

		if (this._is_drag_selectstart) {
			var win = this._getWindow();
	...
	_pGrid.on_fire_user_onkeydown = function (keyCode, altKey, ctrlKey, shiftKey, obj, refer_comp) {
		...
		if (keyCode == nexacro.Event.KEY_ENTER && this._showEditing) {
			...
			var editacceptsenter = edit._cellinfo._getAttrValue(edit._cellinfo.editacceptsenter);

			editacceptsenter = nexacro._toBoolean(editacceptsenter);
			if (edittype == "textarea" && ((!editacceptsenter && (altKey || ctrlKey || shiftKey)) || (editacceptsenter && !altKey && !ctrlKey && !shiftKey))) {
				;
			}
				...
				if ((refer_comp instanceof nexacro.ComboEditCtrl || refer_comp instanceof nexacro.CalendarEditCtrl)
					 && refer_comp.parent._isPopupVisible()) {
					;
				}
				else {
					this._hideEditor();
		...
		if (keyCode == nexacro.Event.KEY_UP) {
			...
			if (!nexacro._enableaccessibility || nexacro._enableaccessibility && this._currentBand == "body") {
				{
					...
					if (refer_comp._type_name == "GridCell") {
						...
						if (editType == "checkbox" && this.currentcol != refer_comp._refobj._col) {
							editType = "";
						}
					}
			...
			if (this.vscrollbar && this.vscrollbar.enable) {
				...
				this.vscrollbar._setPos(newpos, "pageup");
			}
		}
			...
			if (this.vscrollbar && this.vscrollbar.enable) {
				...
				this.vscrollbar._setPos(newpos, "pagedown");
			}
		}
		...
		if (this.getElement() && this._userRowposChange == false) {
			...
			if (cellNewPos < 0) {
				...
				if (this._is_async_recreate) {
					nexacro.OnceCallbackTimer.callonce(this, procClearCellPos, 100);
				}
				else {
					procClearCellPos();
				}
				...
				if (afterRow >= 0 && (this._isMultiSelect() || this._isAreaSelect())) {
					if (obj._bWorkingstatus == true) {
						...
						if (this._is_async_recreate) {
							nexacro.OnceCallbackTimer.callonce(this, procMultiCellPos, 100);
						}
						else {
							procMultiCellPos();
						}
					...
					if (this._is_async_recreate) {
						nexacro.OnceCallbackTimer.callonce(this, procSingleCellPos, 100);
					}
					else {
						procSingleCellPos();
					}
	...
	_pGrid.on_dsnotify_oncolumnchanged = function (obj, e) {
		...
		if (this._isTreeStateChanged(e, this._dsEventOccured) == true) {
			...
			if (this._bodyBand) {
				var matrix = this._bodyBand._matrix;
				if (rowidx == -1 || !matrix._getPhysicalRow(matrix._rows, rowidx)) {
					this._recreate_contents_all(true, true, false, true);
				}
				else {
	...
	_pGrid._callbackPostmsg = function (e) {
		...
		else if (e.id == "showeditor") {
			var _frame = this._getOwnerFrame();
			if (_frame && _frame._activate) {
				if (args[2]) {
					this._showEditorFocus = true;
				}

				this._showEditor(args[0], args[1]);
	...
	_pGrid._afterRowsetChanged = function (kind, updaterow_pos, row, bchange_rowcnt, prev_rowcnt) {
		...
		if (this.getElement()) {
			...
			if (kind == "copydata" || kind == "assign") {
			...
			else if (this._bodyBand && (kind == "addrow" || kind == "appenddata" || kind == "deleterow" || kind == "filterrow" || kind == "insertrow" || kind == "deletemultirows" || kind == "filter")) {
				var body = this._bodyBand;
				var lastrow = this._getDataRow(this._endrowpos);
				var toppos = body._matrix._getBodyRowTopPos(lastrow + 1) - this._getScrollTop();
				var rect = this._getAvailableRect(body);
				...
				if (toppos >= rect.height && lastrow < row && (disp_rows_len % 2 != 1)) {
					if (row <= this._fixed_endrow) {
						...
						if (kind == "deleterow") {
							var disprow = this._dsRowToDispRow(this._binddataset.rowposition);
	...
	_pGrid._updateBodyClient = function (kind, row, chk_srow) {
		...
		if (kind == "insertrow") {
			...
			if (lastPosition != _vpos) {
				...
				if (_vpos >= vlimit && this._scrollpixel != "all") {
					var is_updatecontents = true;
				}

				band._update_rows = band._matrix._adjustScrollRows(_vpos, is_updatecontents);
			}
			...
			if (this._needUpdateExtinner()) {
				...
				if (this.vscrollbar && this.vscrollbar.pos > 0) {
					this.vscrollbar.set_pos(0);
				}
			...
			if (this._needUpdateExtinner()) {
				...
				if (this.autosizingtype == "row") {
					reset_bandsize = true;
				}

				band._matrix._adjustRowsDisplay(reset_bandsize);
				band._matrix._adjustColsDisplay();
			}
	...
	_pGrid._getSelect = function (datarow, nCell) {
		var band;
		...
		return this.isSelectedCell(nCell, band, datarow);
	};
	...
	_pGrid.selectCell = function (nRow, nCellidx, bSelect) {
		if (this._isSelectRowType() || this._isAreaSelect()) {
			return false;
		}
	...
	_pGrid._selectRow = function (row, bSelect, noDraw, cell, bDataset) {
		...
		var retn = false;
		if (bSelect != this._getSelect(row, cell)) {
			if (row == -1) {
				band = "head";
			}
			else if (row == -2) {
	...
	_pGrid._on_focus = function (self_flag, evt_name, lose_focus, refer_lose_focus, new_focus, refer_new_focus) {
		...
		if (evt_name == "tabkey" || evt_name == "shifttabkey") {
			...
			if (nexacro._enableaccessibility) {
				...
				if (self_flag == false) {
					if ((evt_name == "lbuttondown" || evt_name == "rbuttondown") && refer_new_focus && refer_new_focus._type_name == "GridCell") {
						;
					}
					else if (this.autoenter == "select" && evt_name != "rbuttondown") {
						var _win = this._getWindow();
						_win._postMessage(this, "showeditor", this._callbackPostmsg, [false, undefined, true]);
					}
				}
	...
	_pGrid.on_fire_onchar = function (obj, chartext, pretext, posttext, textEvt) {
		var cell = this._currentCellCell;
		var col = this._currentCellCol;
		var pivotindex = this._selectinfo.curpvt;
		var row = this._currentCellRow;
		var subrow = this._currentCellSubRow;

		if (this.onchar && this.onchar._has_handlers) {
			var evt = new nexacro.GridEditCharEventInfo(this, "onchar", cell, chartext, col, pivotindex, posttext, pretext, row, subrow);
			var retn = this.onchar._fireCheckEvent(this, evt);

			textEvt.chartext = evt.chartext;
			return retn;
		}
		return true;
	};

	_pGrid.on_fire_ontextchange = function (obj, chartext, pretext, posttext, preimetext, postimetext, textEvt) {
		...
		if (this.ontextchange && this.ontextchange._has_handlers) {
			...
			var retn = this.ontextchange._fireCheckEvent(this, evt);
	...
	_pGrid._resizeBand = function (reset) {
		...
		var autosizingtype = this.autosizingtype;
		var autosizebandtype = this.autosizebandtype;
		var _is_bodychange = false;

		l = clientleft;
		w = clientwidth;

		if (this._bodyBand) {
			var bodyrowscnt = this._bodyBand._get_rows().length;

			if (bodyrowscnt > 0 && (autosizingtype == "none" || (autosizingtype != "none" && autosizebandtype == "body"))) {
				if (this.summarytype == "top" || this.summarytype == "lefttop") {
		...
		if (this._bodyBand && !_is_bodychange) {
			headHeight = this._getHeadHeight();
			summHeight = this._getSummHeight();
	...
	_pGrid._refreshBodyCell = function (cell, displayrow, clearCurstyle, is_onlycontents) {
		if (!this.enableredraw) {
		...
		if (band) {
			...
			if (rows[displayrow]) {
				...
				band._refreshRowCell(displayrow, cell, selected, undefined, is_onlycontents);
			}
		}
	};
	...
	_pGrid._findCellObj = function (fromComp) {
		...
		while (cellobj && cellobj._type_name != "GridCell") {
			...
			if (cellobj._cellobj && cellobj._cellobj._is_alive && cellobj._cellobj._type_name == "GridCell") {
	...
	_pGrid._isMultiSelected = function () {
		...
		if (this._isSelectRowType()) {
			...
			if (this._selectinfo.rows.length > 1) {
				...
				for (var i = 0, cells_len = cells.length; i < cells_len; i++) {
					if (cells[i]) {
						cnt++;
					}
					if (cnt > 1) {
	...
	_pGrid._resetSelect = function (row, cell, col, subrow, pvt) {
		...
		else if (!this._is_created && row >= 0) {
			this._create_selection = {
				cell : cell, 
				col : col, 
				row : row, 
			...
			if (newPos < 0 && (kind && kind.indexOf("func") < 0)) {
				if (bAllRowDraw) {
					if (!this._clearRows(oldrows)) {
						this._refreshBody(false, !bAllRowDraw);
					}
				}
				else {
				...
				if (bHeadRowDraw) {
					this._refreshHead(false);
				}
				if (bSummRowDraw) {
					this._refreshSumm(false);
				}
			}
	...
	_pGrid._resetRowSizeList = function (chk_srow) {
		...
		if (format._bodyrows) {
			...
			var rowcount = this._rowcount, rows = format._bodyrows, rowsLen = rows.length, _rowSizeListSub = this._rowSizeListSub, _rowSizeList = this._rowSizeList, h, height, index;
			...
			if (this._binddataset && this._bodyAutoSize && (this._autoSizeRowProc || this.autosizingtype == "row" || this.autosizingtype == "both")) {
				for (var i = 0; i < rowcount; i++) {
					...
					for (var j = 0; j < rowsLen; j++) {
						...
						if (keep && _rowSizeListSub[index] >= 0 && this._realrowsized_body[index]) {
							h += _rowSizeListSub[index];
							continue;
						}
						height = this._getMaxSubRowSize(i, j);
						_rowSizeListSub[index] = height;
						h += height;
					}
					_rowSizeList[i] = h;
				}
	...
	_pGrid._isChangeBodyColSizeList = function (columnid, cols, row) {
		...
		for (var i = 0, cells_len = cells.length; i < cells_len; i++) {
			if (cells[i].expr._bindexpr) {
				changesize = cells[i].expr._bindexpr.indexOf(columnid) == -1 ? false : true;
			}
			else if (cells[i].text._bindexpr) {
				if (cells[i].text._bindtype == 2) {
					changesize = cells[i].text._bindexpr.indexOf(columnid) == -1 ? false : true;
				}
				else if (cells[i].text._bindexpr == columnid) {
					changesize = true;
				}
			}
	...
	_pGrid._isChangeBandSrollSizeList = function (band_scroll_sizes) {
		if (this._band_scroll_sizes) {
			var old_sizes_len = this._band_scroll_sizes.length;
			for (var i = 0; i < old_sizes_len; i++) {
				if (this._band_scroll_sizes[i] != band_scroll_sizes[i]) {
					return true;
				}
			}
		}
				...
				if (subcells.length > 0) {
					...
					if (autosizecol == "none") {
						...
						if (displayType == "checkbox") {
							...
							if (maxbyte && colspan == 1 && !this._hasTree) {
								var re_newline = /\r\n|\n|\r/;
	...
	_pGrid._getCellRowTextSize = function (cellinfo, rowidx, text, p_cellinfo) {
		...
		if (displayType == "image") {
			...
			var displayType = cellinfo._getAttrValue(cellinfo.displaytype, rowidx);
			var ctrl_width = 0, ctrl_height = 0;

			var controlborder, controlpadding, linespace;
			var editType = cellinfo._getEdittype(rowidx);

			if (displayType == "combo") {
				var editdisplay = cellinfo._getAttrValue(cellinfo.combodisplay, rowidx);
				if (editdisplay == "display") {
					var ctrl = this.controlcombo;
					controlborder = cellinfo._query_pseudo_control(ctrl, rowidx, "controlborder", "border", "normal");
					controlpadding = cellinfo._query_pseudo_control(ctrl, rowidx, "controlpadding", "padding", "normal");

					if (controlborder) {
						ctrl_width += (controlborder._left_width + controlborder._right_width);
						ctrl_height += (controlborder._top_width + controlborder._bottom_width);
					}
					if (controlpadding) {
						ctrl_width += (controlpadding.left + controlpadding.right);
						ctrl_height += (controlpadding.top + controlpadding.bottom);
					}
				}
			}
			else if (displayType == "date") {
				var editdisplay = cellinfo._getAttrValue(cellinfo.calendardisplay, rowidx);
				if (editdisplay == "display") {
					var ctrl = this.controlcalendar;
					controlborder = cellinfo._query_pseudo_control(ctrl, rowidx, "controlborder", "border", "normal");
					controlpadding = cellinfo._query_pseudo_control(ctrl, rowidx, "controlpadding", "padding", "normal");
				...
				var editdisplay = cellinfo._getAttrValue(cellinfo.editdisplay, rowidx);
				if (editdisplay == "display") {
					var ctrl = this.controledit;

					if (editType == "textarea") {
						ctrl = this._sizetextarea;
						linespace = cellinfo._query_pseudo_control(ctrl, rowidx, "controllinespace", "linespace", "normal");
					}
					...
					if (nexacro.Browser == "Runtime" && (displayType == "normal" || displayType == "text" || displayType == "decoratetext")) {
						linespace = cellinfo._query_pseudo_linespace(rowidx, "normal");
					}
				}
			}
			...
			if (usewordwrap && (this._autoSizeRowProc || this._rowSizeEx) && ((word != "none" && word != false && word != "false") || linespace)) {
				var cols = this._curFormat._cols;
				var col_index = (p_cellinfo ? p_cellinfo._col : 0) + cellinfo._col + cellinfo._colspan - 1;
				var width = cols[col_index].right - cols[cellinfo._col].left;
				...
				if (linespace && this._sizetextarea._input_element) {
					var _linespace = nexacro._parseInt(linespace._value);

					if (nexacro.Browser != "Runtime") {
						if (nexacro.Browser == "Chrome") {
							this._sizetextarea.set_visible(true);
						}
						var size2 = [0, 0];
						this._sizetextarea.move(0, -10, width, 1);
						this._sizetextarea._input_element.setElementSize(width, 1);
						this._sizetextarea.style.set_font(font);
						this._sizetextarea.set_wordwrap(word);
						this._sizetextarea.set_value(text);

						this._sizetextarea._input_element.setElementLineSpace(_linespace);

						size2[0] = this._sizetextarea._input_element.getScrollWidth();
						size2[1] = this._sizetextarea._input_element.getScrollHeight();

						if (this._sizetextarea._client_width < size2[0] && size[0] < size2[0]) {
							size[0] = size2[0];
						}
						if (this._sizetextarea._client_height < size2[1] && size[1] < size2[1]) {
							size[1] = size2[1] + _linespace;
						}

						this._sizetextarea.set_visible(false);
					}
					else {
						var linecnt = nexacro.ceil(size[1] / size1[1]);
						linecnt = (linecnt > 0) ? (linecnt - 1) : 0;
						size[1] += (_linespace * linecnt);
					}
				}
			}
			else {
				...
				if (nexacro.Browser == "Runtime" && linespace) {
					var re_newline = /\r\n|\n|\r/;
					var lines = text.split(re_newline);
					var linecnt = lines.length - 1;

					var _linespace = linespace ? nexacro._parseInt(linespace._value) : 0;
	...
	_pGrid._getMaxSubRowSize = function (rowidx, subrowidx, cells, parentrow, p_cellinfo) {
		var format = this._curFormat;
		var bandrows;
			...
			if (_row == subrowidx || (subcells.length > 0 && _row <= subrowidx && (_row + _rowspan) > subrowidx)) {
				...
				if (subcells.length > 0) {
					...
					if (autosizerow == "none") {
						...
						if (displayType == "checkbox") {
							var controlSize = cells[i]._query_pseudo_control(this, rowidx, "checkboxsize", "buttonsize", "normal");
							...
							var s = this._getCellRowTextSize(cells[i], rowidx, text, p_cellinfo);
							size = s[1];
	...
	_pGrid._exeFuncQueue = function (work) {
		for (var i = 0; i < this._func_queue.length; i++) {
			if (this._func_queue[i].work == work) {
				this._func_queue[i].func.call(this._func_queue[i].pthis, this._func_queue[i].args);
	...
	_pGrid._execRefreshContents = function (workname, bclearcache, noupdatesupp) {
		if (bclearcache && this._recreate_contents_proc.length > 0) {
			this._clearAllStyleCache();
		}

		for (var i = 0; i < this._recreate_contents_proc.length; i++) {
			if (this._recreate_contents_proc[i].workname == workname) {
				this._recreate_contents_proc[i].band._recreate_contents(true, false, false, false, noupdatesupp);
	...
	_pGrid._updateRowSize = function (row, subrow, no_refresh_band) {
		...
		if (row == -1) {
			...
			for (var i = 0, rows_len = rows.length; i < rows_len; i++) {
	...
	_pGrid._refreshCell = function (strBand, nCellIdx, nDisplayRowIdx, clearCurstyle, is_onlycontents) {
		if (!this.enableredraw) {
			this._enable_redraw_history["refreshall"] = true;
			return;
		}
		...
		if (strBand == "head") {
			...
			if (band) {
				...
				if (cell) {
					...
					cell._updateAll(undefined, is_onlycontents);
				}
			}
		}
		else if (strBand == "summ" || strBand == "summary") {
			...
			if (nDisplayRowIdx >= 0) {
				this._refreshBodyCell(nCellIdx, nDisplayRowIdx, clearCurstyle, is_onlycontents);
			}
			else {
				for (var i = 0; i < this._getDispRowCnt(); i++) {
					this._refreshBodyCell(nCellIdx, i, clearCurstyle, is_onlycontents);
				}
			}
		}
	};
					...
					for (var i = 0; i < cellsLen; i++) {
						...
						if (cellinfo._col <= nColIndex && (cellinfo._col + cellinfo._colspan) > nColIndex) {
							...
							if (cellinfo._subcells.length) {
								...
								for (var j = 0, subcells_len = subcells.length; j < subcells_len; j++) {
	...
	_pGrid._currentCellCol = -1;
	_pGrid._currentCellSubRow = -1;
	...
	_pGrid._preloadTreeImage = function () {
		...
		for (var i = 0; i < rowcount; i++) {
			for (var j = 0, cellinfos_len = cellinfos.length; j < cellinfos_len; j++) {
	...
	_pGrid._preloadImage = function () {
		if (this._image_load_all != null || this.autosizingtype == "none") {
			return true;
		}
	...
	_pGrid._moveToCell = function (type, editable, colmove, area, lastcol, showcell) {
		...
		var format = this._curFormat;
		if (format == null || !format._bodycells) {
			return false;
		}
		...
		if (type == "next") {
				...
				if (this._selectinfo.curcell >= bodycells.length - 1) {
					...
					for (; true; ) {
						...
						if (this._hasTree) {
							...
							for (var k = 0, treeidx_len = _treeIndexes.length; k < treeidx_len; k++) {
			...
			for (; true; ) {
				...
				if (this._hasTree) {
					...
					for (var k = 0, n = _treeIndexes.length; k < n; k++) {
						if (_treeIndexes[k] == afterRow) {
							exist = true;
							break;
						}
		...
		this._hideEditor();
		var selectinfo = this._selectinfo;
		var change = this._ChangeSelect(selectinfo.curcell, selectinfo.curcol, selectinfo.curdsrow, selectinfo.cursubrow, selectinfo.curpvt, false, beforeCell, beforeCol, beforeRow, beforeSubrow, beforePvt, "body", evt_name);

		var cell = this._getCurrentBodyCell(selectinfo.curdsrow, selectinfo.curcell);

		if (cell && !nexacro._enableaccessibility) {
			cell._no_dom_focus = true;
			cell._setFocus(false);
			cell._no_dom_focus = false;
		}
	...
	_pGrid._needAfterMoveToCell = function () {
		if (!this._keydown_elem) {
			this._moveCellAfterFocus();
		}
		else {
			if (this.autoenter == "select" && !this._showEditing) {
				var win = this._getWindow();
				win._postMessage(this, "showeditor", this._callbackPostmsg, [false, true]);
			}
		}
	};
	...
	_pGrid._moveCellAfterFocus = function () {
		...
		if (this.autoenter == "select") {
			...
			if (this._currentBand == "body" && !this._showEditing) {
				...
				if (this._beforeEditRowIdx != currow || this._beforeEditCellIdx != curcell) {
					if (cellobj._hasEditor()) {
						...
						if (editor) {
							...
							else if (editor.comboedit && editor.comboedit.setCaretPos && !editor.comboedit.autoselect) {
	...
	_pGrid._hideEditor = function (flag, noApplyDataset, grid_killfocus) {
		if (!flag) {
			...
			if (editComp) {
				...
				if (cellobj) {
					if (!noApplyDataset) {
						...
						if (this._currentCellEditor && this._currentCellEditor != editComp) {
			...
			this._currentCellCol = -1;
			this._currentCellSubRow = -1;
		}
		else {
			var editComp = this._currentCellEditor;
			if (editComp) {
	...
	_pGrid._createEditor = function (cellobj) {
		...
		if (this._tempEditor) {
			this._clearDestroyEditor(true);
			this._tempEditor.set_visible(false);
	...
	_pGrid._createEditor_button = function (cellobj) {
		...
		if (nexacro.Browser == "IE" && !nexacro._enableaccessibility) {
			nexacro.OnceCallbackTimer.callonce(this, function () {
	...
	_pGrid._createEditor_maskedit = function (cellobj, type) {
		...
		var editautoselect = cMaskEdit._cellinfo._getAttrValue(cMaskEdit._cellinfo.editautoselect, datarow);
		...
		if (editautoselect) {
			cMaskEdit._is_first_focus = false;
		}
	...
	_pGrid._createEditor_date = function (cellobj) {
		...
		var editautoselect = cCalendar._cellinfo._getAttrValue(cCalendar._cellinfo.editautoselect, datarow);
		...
		if (editautoselect) {
			cCalendar.calendaredit._is_first_focus = false;
		}
		return cCalendar;
	};
	...
	_pGrid._createEditor_textarea = function (cellobj, readonly) {
		...
		var editacceptsenter = cellinfo._getAttrValue(cellinfo.editacceptsenter);
		cTextArea._is_use_ex_enter = !nexacro._toBoolean(editacceptsenter);
	...
	_pGrid._setSelectBorder = function () {
		if (!this._is_created) {
			return;
		}
		...
		var band = this._bodyBand;
		if (band) {
			var selectborder = band.on_find_CurrentStyle_selectborder(band._pseudo);
		}

		if (!band || !selectborder) {
			this._destroySelectBorderElements();
			return;
		}
		...
		if (this._isAreaSelect() || this._isMultiSelect()) {
			var cellobj = null;
			var selected_area = selectinfo.area;
			for (var i = 0, area_len = selected_area.length; i < area_len; i++) {
				sub_elements = [];
				for (var row = selected_area[i].begrow, e_row = selected_area[i].endrow; row <= e_row; row++) {
					var col = selected_area[i].begcol;
					var e_col = selected_area[i].endcol;


					if (this._isSelectRowType()) {
						var rowobj = band._get_row(row);
						if (rowobj && rowobj._cells) {
							e_col = rowobj._cells.length - 1;
						}
						col = 0;
					}

					for (; col <= e_col; col++) {
						cellobj = this._getCurrentBodyCell(row, col);
						if (cellobj) {
							if (cellobj._subComp) {
								sub_elements.push(cellobj._subComp._control_element);
							}
							sub_elements.push(cellobj._control_element);
						}
					}
				}
			...
			var rows = selectinfo.rows;
			var row = band._get_row(rows[0]);
			var selected_cells = selectinfo.getSelectCells(rows[0]);
			if (row && row._cells) {
				var cells = row._cells;
				for (var j = 0, cells_len = cells.length; j < cells_len; j++) {
					if (selected_cells[j]) {
						if (cells[j]._subComp) {
							if (cells[j]._subComp instanceof nexacro.GridControlTree) {
								if (cells[j]._subComp._btn_elem) {
									sub_elements.push(cells[j]._subComp._btn_elem);
								}
								if (cells[j]._subComp._chk_ctrl) {
									sub_elements.push(cells[j]._subComp._chk_ctrl._control_element);
								}
								if (cells[j]._subComp._img_elem) {
									sub_elements.push(cells[j]._subComp._img_elem);
								}
								if (cells[j]._subComp._text_elem) {
									sub_elements.push(cells[j]._subComp._text_elem);
								}
								sub_elements.push(cells[j]._subComp._control_element);
							}
							else {
								sub_elements.push(cells[j]._subComp._control_element);
							}
						}
						sub_elements.push(cells[j]._control_element);
					}
				}
		...
		if (elements && elements.length > 0) {
			...
			var selectborder_elements = this._selectborder_elements;
			for (var i = 0, len = elements.length; i < len; i++) {
				sub_elements = elements[i];
				if (sub_elements && sub_elements.length > 0) {
					if (!selectborder_elements[elements_index]) {
						var elem = new nexacro.EventPassOverlayElement(this._bodyBand._control_element, sub_elements);
						elem.create();
						selectborder_elements.push(elem);
					}
					else {
						var elem = selectborder_elements[elements_index];
						elem.setTargetElements(sub_elements);
					}

					var start_cell = elem._findLinkedControl(sub_elements[0]);
					if (!(start_cell instanceof nexacro.GridCell)) {
						start_cell = this._findCellObj(start_cell);
					}

					var end_cell = elem._findLinkedControl(sub_elements[sub_elements.length - 1]);
					if (!(end_cell instanceof nexacro.GridCell)) {
						end_cell = this._findCellObj(end_cell);
					}

					var s_band_rect = start_cell.parent._getAreaRect(start_cell._refobj._area);
					var e_band_rect = end_cell.parent._getAreaRect(end_cell._refobj._area);

					var scroll_left = this._getScrollLeft();
					var scroll_top = this._getScrollTop();

					var left = (s_band_rect.left + start_cell.parent._adjust_left + start_cell._adjust_left) - scroll_left;
					var top = (s_band_rect.top + start_cell.parent._adjust_top + start_cell._adjust_top) - scroll_top;

					var width = (e_band_rect.left + end_cell.parent._adjust_left + end_cell._adjust_left + end_cell._adjust_width - scroll_left) - left;
					var height = (e_band_rect.top + end_cell.parent._adjust_top + end_cell._adjust_top + end_cell._adjust_height - scroll_top) - top;

					elements_index++;

					var adjust_width = 0, adjust_height = 0;
					var l_border_hidden = false, t_border_hidden = false, r_border_hidden = false;

					var start_area = start_cell._refobj._area;
					var end_area = end_cell._refobj._area;


					if (start_area == "left" || start_area == "right") {
						left = left + scroll_left;

						if (end_area != "left" && end_area != "right") {
							width = width - scroll_left;
							adjust_width = s_band_rect.width - left;

							if (adjust_width >= width) {
								width = adjust_width;
							}

							if (left + width >= e_band_rect.left + e_band_rect.width) {
								adjust_width = (left + width) - (e_band_rect.left + e_band_rect.width);
								width = width - adjust_width;

								if (adjust_width > selectborder._right_width) {
									r_border_hidden = true;
								}
							}
						}
					}
					else if (start_area == "body") {
						if (end_area == "right") {
							adjust_width = scroll_left;
							width = width + adjust_width;

							if (left >= e_band_rect.left) {
								left = e_band_rect.left;
								width = end_cell._adjust_left + end_cell._adjust_width;
							}

							if (left <= s_band_rect.left && 0 < s_band_rect.left) {
								adjust_width = s_band_rect.left - left;

								left = s_band_rect.left;
								width = width - adjust_width;

								if (adjust_width > selectborder._left_width) {
									l_border_hidden = true;
								}
							}
						}
						else if (end_area == "body") {
							if (left <= s_band_rect.left && 0 < s_band_rect.left) {
								adjust_width = s_band_rect.left - left;
							...
							if (left + width >= s_band_rect.left + s_band_rect.width) {
								adjust_width = (left + width) - (s_band_rect.left + s_band_rect.width);
								width = width - adjust_width;
					...
					if (this._fixed_height > 0) {
						if (this._fixed_endrow >= start_cell._rowidx) {
							top = top + scroll_top;
							if (this._fixed_endrow < end_cell._rowidx) {
								adjust_height = this._fixed_height - scroll_top;
								height = height + adjust_height;

								adjust_height = this._fixed_height - top;
							...
							top = top + this._fixed_height;
							if (top <= this._fixed_height) {
								adjust_height = this._fixed_height - top;

								top = this._fixed_height;
								height = height - adjust_height;

								if (adjust_height >= selectborder._top_width) {
									t_border_hidden = true;
								}
							}
						}
					}
					...
					left = (s_band_rect.left > 0 || left > 0) ? left - 1 : left;
					...
					elem._setElementBorder(selectborder, l_border_hidden, t_border_hidden, r_border_hidden);

					sub_elements = null;
					sub_elements = [];
				}
			}
			this._selectborder_elements = selectborder_elements;
			selectborder_elements = null;
		}
		else {
			this._destroySelectBorderElements();
		}
		...
		if (elements_index < this._selectborder_elements.length) {
			this._destroySelectBorderElements(elements_index);
		}
	};

	_pGrid._destroySelectBorderElements = function (start_index) {
		var selectborder_elements = this._selectborder_elements;
		if (selectborder_elements.length) {
			var index = start_index | 0;
			var len = selectborder_elements.length;

			for (; index < len; index++) {
				selectborder_elements[index].destroy();
				selectborder_elements[index] = null;
			}
			selectborder_elements.splice(start_index | 0, len);
		}
		this._selectborder_elements = selectborder_elements;
	};
	...
	_pGrid._setOverlayElementProperty = function (elem, left, top, width, height, style_cells, display_text, is_fake_merge) {
		...
		if (style_cells && style_cells.length > 0) {
			...
			var datarow = this._getDataRow(cellobj._rowidx);
			var celltext = cellobj._getDisplayText();
			var display_type = cellinfo._getDisplaytype(this._getDataRow(cellobj._rowidx));
			if (display_type == "image") {
				elem.setElementText("");
				elem.setElementDecorateText("");
				elem.setElementImageUrl(celltext);
			}
			else {
				elem.setElementImageUrl("");
				...
				if (display_type == "decoratetext") {
					elem.setElementDecorateText(celltext);
				}
				else {
					if (elem.decoration != "") {
						elem.decoration = "";
			...
			elem.updateCellNodeClient(left, top, width, height, cell_hpos, adjust_pos);
			cellobj.selected = selected;
		}
	};

	_pGrid._adjustOverlayElements = function (is_create, fake_merge, is_scrolling) {
							...
							if (suppinfo && suppinfo.last) {
								...
								if (elements.length > 0) {
									if (overlay_elements[overlay_index]) {
										...
										elem = new nexacro.EventPassOverlayElement(style_cells[0]._band._control_element, elements);
										elem.create();
										overlay_elements.push(elem);
									}

									if (elem) {
										adjust_left = style_cells[0]._band._adjust_left;
										adjust_top = style_cells[0]._band._adjust_top;

										overlay_index++;
										this._setOverlayElementProperty(elem, left - adjust_left, top - adjust_top, width, height, style_cells, display_text);
									}
								}
								else {
		...
		if (this._is_use_fakemerge && fake_mergecell_arr.length) {
			...
			for (var i = 0, arr_len = fake_mergecell_arr.length; i < arr_len; i++) {
				elements = [];
				style_cells = [];
				...
				for (; j <= end_column; j++) {
					...
					for (var k = start_row; k <= end_row; k++) {
						...
						while (true) {
							...
							if (!cellobj || cellobj._refobj._row > subrow_end) {
								break;
							}
				...
				if (style_cells.length > 0) {
					if (1) {
						...
						this._setOverlayElementProperty(elem, left - adjust_left, top - adjust_top, width, height, style_cells, display_text, true);
	...
	_pGrid._moveToPosAccessibilityCell = function (rowidx, cellidx) {
		...
		if (cellobj && nexacro._enableaccessibility) {
			var cellinfo = cellobj._refobj;
