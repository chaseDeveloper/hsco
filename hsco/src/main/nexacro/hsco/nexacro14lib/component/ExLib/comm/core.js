...
if ( !JsNamespace.exist("Ex.core")  )
{
	...
	JsNamespace.declare("Ex.core",{
		...
		*/
		...
		{
			return Ex.util.getLookupData(application.gds_menu, this.ct_menuIdCol, menuid, menuInfo);
		},
		...
		*/
		...
		{
			var sSvcID        	= "user";                    
		    var sController   	= "authorGroupList.do";
		...
		*/
		tran : function()
		{
			...
			argument += " SYS_ID='DUCO'";
			argument += " MENU_ID=" + application.gv_curFormId.split("_")[1] + "";
			*/
			...
			*/
			/************************************************************************
			*pThis.dsTransInfo, pThis.dsParamINfo의 데이터가 null일경우가 발생한다..
			*예외 사항을 처리가 구현되어 있었으나 적용되어 있지 않음..
			*예외 사항 적용함.. 추후 문제 발생시 원복...
			*수정자 : 지효정 
			/************************************************************************/
			/*
			pThis.dsTransInfo = _dsTransInfo; 
			pThis.dsParamINfo = _dsParamInfo; 
			...
			for (var i = 0; i < dsInputName.length ; i++ ){
				...
				if (i == 0)
				{
					...
					pThis.dsTransInfo.setColumn(row,"MENU_ID", "value");
				}
			...
			*/
			...
			var baseTranInfo = this.setTranBaseInfo(pThis, svcID, sController, inDatasets, outDatasets, argument);
			...
			inDatasets  =  baseTranInfo["strInDatasets"] +  inDatasets;
			...
			var strArgument = baseTranInfo["arguments"];
		...
		/**
		 * Tran 정보 구성
		 *  - Ex.core.tran 에 있던 내용을 File Upload에서도 동일하게 처리를 위해 별도 함수로 분리함
		 */
		setTranBaseInfo :function(pThis, svcID, sController, inDatasets, outDatasets, argument){
			...
			if(Eco.isEmpty(Ex.util.ffobj)){
				Ex.util.ffobj = pThis;
			}
			
			var _dsTransInfo = Ex.util.isCheckDs("dsTransInfo");
			var _dsParamInfo = Ex.util.isCheckDs("dsParamINfo");		//argument --- > dataset으로 처리
			...
			*수정자 : 지효정 
			...
			result = {};
			result["strInDatasets"] = "__DS_PARAM_INFO__=dsParamINfo __DS_TRANS_INFO__=dsTransInfo gds_userInfo=gds_userInfo gds_pageInfo=gds_pageInfo ";
