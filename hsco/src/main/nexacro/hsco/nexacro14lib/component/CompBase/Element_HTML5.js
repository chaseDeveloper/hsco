	...
	if (!nexacro.Element) {
		...
		if (nexacro.Browser == "IE" && nexacro.BrowserVersion < 9) {
			...
			_pInputElement._createInputHandle = function () {
				...
				if (elem_handle) {
					...
					if ((nexacro.Browser == "Edge" || (nexacro.Browser == "IE" && nexacro.BrowserVersion == 11)) && nexacro.SystemLang == "ko-KR") {
						if (this.type) {
		...
		if (nexacro.SupportTouch && nexacro.isTouchInteraction) {
			_pInputElement.setElementInputType = function (type, bkeypad) {
				...
				if (this._type != type) {
					...
					if (input_handle) {
						nexacro.__changeInputDOMNodeType(input_handle, this._type);
						if (nexacro.OS == "iOS" && (this._keypad_type == "change" || this._keypad_type == "changed")) {
							this.setElementValue(this.text);
						}
					}
				}
			};
			...
			_pInputElement.setElementInputTypeKeypad = function (type) {
				if (type == "string") {
					...
					if (nch < 0) {
						this._keypad_type = "number";
					}
					else {
						this._keypad_type = "text";
					}
				}
				else if (type == "change" || type == "changed") {
		...
		_pInputElement._on_sys_focus = function (target) {
			...
			if (_win._is_active_window) {
				...
				if (nexacro.OS == "iOS" && this._keypad_type == "change" && !this.readonly) {
					var text = comp.text;
					var ppThis = this;
					setTimeout(function () {
						editbase._text = text;
						ppThis.setElementInputType("text", true);

						if (comp.autoselect) {
							var selecttext = editbase._select_text = editbase._text;
							editbase._is_apply_autoselect = true;
							editbase.setElementCaretPos(0, selecttext.length, ppThis);
						}
						ppThis.setElementInputTypeKeypad("changed");
					}, 500);
				}
		...
		_pInputElement._on_sys_blur = function (target) {
			...
			if (nexacro.OS == "iOS" && (this._keypad_type == "changed" || this._keypad_type == "change")) {
				this.setElementInputTypeKeypad(this._orgtype);
		...
		_pInputElement._on_sys_touchend = function () {
			if (nexacro.OS == "iOS") {
				if ((this._keypad_type == "number" || this._keypad_type == "digit") && !this._tap_blocked && !this._is_longpressed && !this.readonly) {
					if (this._is_focused && this._is_focusing) {
						var comp = this._parent_elem.linkedcontrol;
						var editbase = comp._edit_base_api;
						var text = comp.text;
						var ppThis = this;
					...
					if (this._keypad_type == "number") {
						this.setElementInputType("number", true);
					}
					else if (this._keypad_type == "digit") {
						this.setElementInputType("digit", true);
					}
					this.setElementInputTypeKeypad("change");
				}
		...
		nexacro._inputhandler_onmouseup = function (node, evt) {
			...
			if (elem && elem._on_sys_mouseup) {
				...
				if (evt.button != 2 && elem._useTimer) {
					...
					nexacro.OnceCallbackTimer.callonce(comp, function () {
						if ((nexacro.Browser != "Edge" && nexacro.Browser != "IE") && nexacro._getSysEventBtnCode(evt) == (nexacro._is_attach_childframe ? nexacro_HTMLSysEvent[id].MOUSE_LBUTTON : nexacro_HTMLSysEvent.MOUSE_LBUTTON)) {
							evt.id = id;
		...
		if (nexacro.Browser == "Chrome") {
			_pControlElementBase._refreshForeground = function (_handle, handle_style) {
				...
				if (!this.visible) {
					nexacro.__setDOMNodeStyleDisplay(_handle.style, "none");
				}
				if (!this.enable) {
		...
		_pPluginElement.callMethod = function () {
			...
			if (this._handle && this._object_node) {
				...
				if (nexacro.Browser != "Chrome" && this.plugintype == "application/x-shockwave-flash") {
					if (this._embed_node) {
		...
		_pWebBrowserPluginElement.destroy = function () {
			...
			if (_handle) {
				...
				if (this._parent_dest_handle) {
					var node_child = null;
					node_child = this._document ? (this._document.childNodes ? this._document.childNodes[0] : null) : null;
					if (node_child) {
						nexacro.__removeAllDOMNode(node_child);
						nexacro.__removeDOMNode(this._document, node_child);
						node_child = null;
					}
