...
if (!nexacro.Component) {
		...
		if (this.visible != v) {
			...
			if (!v && this._is_created && this.parent) {
				if (_window && _window._indexOfCurrentFocusPaths(this) > -1) {
					...
					this.tabstop = false;
					newfocus_comp = _form._searchNextTabFocus(_form._last_focused);
				...
				if (this.visible) {
					...
					if (!this._is_subcontrol && this._is_created && parent && parent._is_created) {
						if (_window && _window._focus_list && _window._indexOfCurrentFocusPaths(parent) == _window._getCurrentFocusPathsLength() - 1) {
							var _form = this._getForm();
							if (!_form || (_form && !_form._last_focused)) {
								this._on_focus(true);
							}
						}
					}
	...
	_pComponent.set_enable = function (v) {
		...
		if (this.enable != v) {
			...
			if (this._is_created) {
				...
				if (this instanceof nexacro.MainFrame) {
					enable_flag = v;
				}
	...
	_pComponent.show = function () {
		if (nexacro._load_session_timer) {
			nexacro._load_session_timer.stop();

			if (nexacro._load_session_timer.callback) {
				nexacro._load_session_timer.callback.call();
			}
		}
