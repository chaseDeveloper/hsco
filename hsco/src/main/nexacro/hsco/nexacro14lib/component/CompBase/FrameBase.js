...
if (!nexacro.Frame) {
	...
	_pFrame._on_titlebar_movetrack = function (x, y, dragdata, windowX, windowY) {
		...
		if (windowX != undefined && windowY != undefined) {
			var amleft = parseInt(_mainframe.left) ? parseInt(_mainframe.left) : 0;
			var amtop = parseInt(_mainframe.top) ? parseInt(_mainframe.top) : 0;

			var winX = (windowX ? windowX : 0) + amleft;
			var winY = (windowY ? windowY : 0) + amtop;
			var r = amleft + _mainframe._adjust_width;
			var b = amtop + _mainframe._adjust_height;

			if (!(_mainframe.left <= winX && r >= winX && _mainframe.top <= winY && b >= winY)) {
				return;
			}
		}

		var width = _pos.width ? _pos.width : (this.form ? this.form._adjust_width : 0);
		var height = _pos.height ? _pos.height : (this.form ? this.form._adjust_height : 0);
		if (!this._is_window) {
			this._move(_pos.left + x, _pos.top + y, width, height);
		}
		else if (this._is_window && (this._window_type == 5 || this._window_type == 2)) {
			this._move(this._left + x, this._top + y, width, height);
		}
