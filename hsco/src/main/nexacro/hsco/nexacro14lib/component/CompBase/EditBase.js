...
if (!nexacro.EditBase) {
	...
	_pEditBase._on_default_input_select = function (elem) {
		if (this._accept_select_event === false) {
			...
			if (nexacro.Browser == "Chrome" || nexacro.Browser == "Gecko") {
				var pos = elem.getElementCaretPos();

				if (!(pos.begin == this._begin_pos && pos.end == this._end_pos)) {
	...
	if (nexacro.Browser == "IE" && nexacro.BrowserVersion <= 10 && nexacro._getRealBrowserVersion() <= 10) {
		if (nexacro.OSVersion >= 6.0) {
			if (nexacro.Browser == "IE" && nexacro.BrowserVersion == 10) {
				_pEditBase._on_default_input_keyinput = function (elem) {
					var mouse_stat = this._stat_mouse.getCurrentStatus();
					if (mouse_stat == "mousedown" && !(nexacro.Browser == "Edge" && this._is_composition())) {
					...
					if (this._is_selected() && this._select_text == "") {
		...
		if (major_ver < 7) {
			...
			_pEditBase._on_default_input_keyinput = function (elem) {
				...
				if (this._charcode >= 12593 && this._charcode <= 12643) {
					if (!this._is_composition()) {
						this._imefirsttxt = this._text;
						this._set_input_begin_pos((!pos || pos == -1) ? 0 : pos.begin - 1);
						var data = this.getCompositionData(elem);
						this._on_input_compositionstart(data);
					}
					else {
				...
				else if (this._is_composition() && this._charcode == 0 && ((major_ver >= 10 && minor_ver >= 3) || major_ver >= 11)) {
					...
					if (data != this._stat_composition.getData()) {
				...
				if (this._is_ios_paste_composition) {
					...
					var begin = (this._text == this._textEventInfo.prechareventtext) ? this._begin_pos - insertText.length : this._begin_pos;
					var pThis = this;
				...
				if (this._pasteAction && this._stat_composition.getPreviousStatus() == nexacro.EditBase.Status.CompositionUpdate) {
					...
					if (!this._is_hangul(prev_data)) {
						...
						this.setElementCaretPos(pos.end, pos.end);
						this._input_begin_pos = pos.end - insertText.length;
		...
		_pEditBase._on_default_input_keyinput = function (elem) {
			...
			if (this._is_hangul(insertText) && diff > insertText.length) {
			...
			if (this._pasteAction && this._stat_composition.getCurrentStatus() == nexacro.EditBase.Status.CompositionEnd && this._stat_composition.getPreviousStatus() == nexacro.EditBase.Status.CompositionUpdate) {
		...
		_pEditBase._on_default_input_keyinput = function (elem) {
			...
			var insertText = this.getInsertText(elem);
			if (nexacro.Browser == "Runtime" || nexacro.Browser == "Edge") {
				if (this._text.length == elem_value.length && this._text != elem_value) {
					var replaceText = elem_value.substr(this._begin_pos, elem_pos.begin - this._begin_pos);
					if (replaceText.length > 0) {
						var temp = replaceText;
			...
			if (nexacro.OS == "Android" && nexacro.Browser == "Chrome") {
				if (comp.maxlength > 0 && this._select_text == "" && this._focus_value && this._focus_value.length >= comp.maxlength) {
					...
					if (this._is_composition()) {
						...
						if (!this._pasteAction) {
							this._input_begin_pos = this._mobile_elme_pos = elem_pos.begin - 1;
							this._is_mobile_drag = true;
						}
					}
					input_text = null;
			...
			if (nexacro.Browser == "Chrome" || nexacro.Browser == "Runtime") {
		...
		_pEditBase._on_default_input_compositionupdate = function (data) {
			...
			if (data == "" || this._is_hangul(data)) {
				...
				if (this._stat_composition.getCurrentStatus() == nexacro.EditBase.Status.CompositionStart) {
					this._set_input_begin_pos(pos.end - data.length);
	...
	if (nexacro.Browser == "Edge" || (nexacro.Browser == "IE" && (nexacro.BrowserVersion == 11 || (nexacro.BrowserType == "IE10" && nexacro._getRealBrowserVersion() == 11)))) {
	...
	else if (nexacro.Browser == "Chrome" && nexacro.BrowserVersion >= 53) {
		_pEditBase._on_default_input_compositionend = function (data) {
			this._compositionstart_value = "";
			this._stat_composition.setStatus(nexacro.EditBase.Status.CompositionEnd, data);
	...
	if (nexacro.Browser == "IE" && nexacro.BrowserVersion == 11) {
		...
		_pEditBase._isJapaneseReComposition = function () {
			...
			if (!this._is_hangul(data) && curr_stat == nexacro.EditBase.Status.CompositionStart && data != "" && (this._input_begin_pos == this._begin_pos) && (this._begin_pos == this._end_pos) && (this._begin_pos != pos.begin)) {
				data = status = pos = null;
				return true;
			}
	...
	_pEditNormal._on_default_input_keydown = function (elem, keyCode, altkey, ctrlkey, shiftkey) {
		...
		if (keyCode == nexacro.KeyCode_ImeInput) {
			...
			if (this._is_composition() && ((keyCode == 8 && (nexacro.Browser != "MobileSafari" || (nexacro.Browser == "MobileSafari" && this._is_hangul(this.getCompositionData(elem)))))
				 || keyCode == 46 || (nexacro.OS != "iOS" && keyCode == 32) || (keyCode >= 33 && keyCode <= 40))) {
		...
		var movefocus = false;
		if (keyCode == 9 && comp && comp.acceptstab == false) {
			movefocus = true;
		}
	...
	_pEditNormal._on_default_input_keypress = function (elem, keyCode, charCode, altKey, ctrlKey, shiftKey) {
		...
		if ((nexacro.OS == "iOS" || nexacro.Browser == "Edge") && (k == nexacro.Event.KEY_ENTER)) {
		...
		if (bFireEvt) {
			...
			if (bEvtRet) {
				...
				if (bEvtRet || !bEvtRet && is_ios_composition) {
					if (bEvtRet) {
						if ((nexacro.OS == "iOS" && nexacro.Browser == "MobileSafari") || nexacro.Browser == "Edge") {
							var charEvt = new nexacro.TextChangeEventInfo(comp, "onchar", text_info.chartext, text_info.prechareventtext, text_info.posttext);
	...
	_pEditNormal._on_default_input_keyup = function (elem, keycode, altkey, ctrlkey, shiftkey) {
		...
		if (k == 8) {
			if (nexacro.Browser == "IE" && nexacro.BrowserVersion == 11 || nexacro.Browser == "Edge") {
				if (this._is_composition()) {
		...
		if (nexacro.OS == "Android" && nexacro.Browser == "Chrome" && this._is_mobile_drag) {
			var _lg_flag = window.navigator.appVersion.indexOf("LG");
	...
	_pEditNormal._blur_process = function (elem) {
		if (this._is_composition()) {
			...
			if (nexacro.Browser == "Edge" && data == "" && this._is_hangul(this._stat_composition.getData())) {
				data = this._stat_composition.getData();
			}
			...
			if (nexacro.OS == "iOS" && nexacro.Browser == "MobileSafari" && elem._is_ios_composition_returned) {
				var maxlength = this.comp.maxlength;
		...
		if (nexacro.OS == "iOS" && (elem._keypad_type == "change" || elem._keypad_type == "changed")) {
			elem.setElementInputTypeKeypad(elem._orgtype);
			elem._tap_blocked = false;
			elem._is_longpressed = false;
		}

		if (nexacro.Browser == "Edge" && !this._is_composition()) {
			var data = this._stat_composition.getData();
	...
	_pEditNormal._mouseup_process = function (elem) {
		...
		if (this._is_composition() || ((caret.begin != this._begin_pos && caret.end != this._end_pos) && nexacro.Browser != "Runtime")) {
			...
			if (nexacro.Browser == "Edge" && caret.begin == caret.end) {
	...
	if (nexacro.Browser == "Edge" || nexacro.Browser == "IE") {
		...
		_pEditMaskString._setElementValue = function (elem, begin, end, bApply) {
			...
			if (bApply) {
				if (nexacro.Browser == "Chrome" && nexacro.OSVersion >= 10.0) {
					this.setElementCaretPos(begin, end, elem);
					var pThis = this;
					nexacro.OnceCallbackTimer.callonce(this.comp, function () {
						pThis._accept_focus_event = false;
				...
				else if (nexacro.OS == "Android" && (nexacro.OSVersion == "4.4.2" || nexacro.OSVersion == "6.0.1" || nexacro.OSVersion == "7.0") && nexacro.Browser == "Chrome") {
					this.setElementCaretPos(begin, end, elem);
					var pThis = this;
					nexacro.OnceCallbackTimer.callonce(pThis.comp, function () {
						pThis._accept_focus_event = false;
	...
	_pEditMaskNumber.removeMask = function (str, bValue) {
		...
		if (bValue) {
			for (i = 0; i < ntxtLen; i++) {
				...
				else if (c == this._default_decimal_point && bPoint === false) {
					buf.push(c);
					bPoint = true;
					bInside = true;
				}
				else if (c != this._default_thousands_sep) {
					if (!this._pasteAction) {
						return "";
					}
				}
	...
	_pEditMaskNumber.onInputDigit = function (strText, xchNum, nCaret) {
		...
		if (this._text && elem_val.length == this._text.length) {
			if (!this._pasteAction) {
	...
	_pEditMaskNumber._fire_text_event = function (chartext) {
		...
		if (bFilter || changeChartext) {
			...
			if (nexacro.OS == "Mac OS" && bFilter && this._stat_composition._curr_stat == nexacro.EditBase.Status.CompositionEnd && !this._is_hangul(chartext)) {
				var pThis = this;
				nexacro.OnceCallbackTimer.callonce(pThis.comp, function () {
			...
			else if (nexacro._isDesktop() && nexacro.Browser == "Safari" && bFilter && this._stat_composition._curr_stat == nexacro.EditBase.Status.CompositionEnd && !this._is_hangul(chartext)) {
				var pThis = this;
				nexacro.OnceCallbackTimer.callonce(pThis.comp, function () {
		...
		if (nexacro._isDesktop() && nexacro.Browser == "Safari") {
