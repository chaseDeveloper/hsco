...
if (nexacro.Component) {
	...
	_pComponent._on_dblclick = function (elem, button, alt_key, ctrl_key, shift_key, canvasX, canvasY, screenX, screenY) {
		...
		if (!pThis.onlbuttonup || (pThis.onlbuttonup && !pThis.onlbuttonup.defaultprevented)) {
			if (this.visible && this._isEnable() && this.enableevent) {
				...
				this.on_dblclick_basic_action(elem, button, alt_key, ctrl_key, shift_key, canvasX, canvasY, screenX, screenY);
			}
		}
	};

	_pComponent.on_dblclick_basic_action = function (elem, button, alt_key, ctrl_key, shift_key, canvasX, canvasY, screenX, screenY) {
	...
	_pComponent._on_bubble_touch_lbuttondown = function (elem, button, alt_key, ctrl_key, shift_key, canvasX, canvasY, screenX, screenY, event_bubbles, fire_comp, refer_comp, bubble_scope, isTap) {
		...
		if (event_bubbles === undefined) {
			...
			if ((this._is_popup_frame && this._is_window)) {
				return;
			}
			if ((!this.onlbuttondown || (this.onlbuttondown && !this.onlbuttondown.stoppropagation)) && (event_bubbles !== true) && this.parent && !this.parent._is_application) {
				var canvas = this._getRecalcCanvasXY(elem, canvasX, canvasY);
	...
	_pComponent._on_bubble_mousemove = function (elem, button, alt_key, ctrl_key, shift_key, canvasX, canvasY, screenX, screenY, event_bubbles, fire_comp, refer_comp, bubble_scope) {
		...
		if (event_bubbles === undefined) {
			...
			if (this.visible && this._isEnable()) {
				if (bubble_scope) {
					this.on_mousemove_basic_action(is_subcontrol_bubble);
				}
	...
	_pComponent._on_bubble_mouseenter = function (elem, from_comp, button, alt_key, ctrl_key, shift_key, canvasX, canvasY, screenX, screenY, event_bubbles, fire_comp, refer_comp, bubble_scope) {
		...
		if (event_bubbles === undefined) {
			...
			if (this.visible && this._isEnable()) {
				this.on_mouseenter_basic_action(is_subcontrol_bubble);
	...
	_pComponent._on_dragmove = function (elem, src_comp, src_refer_comp, dragdata, userdata, button, alt_key, ctrl_key, shift_key, canvasX, canvasY, screenX, screenY, event_bubbles, fire_comp, refer_comp, xdeltavalue, ydeltavalue) {
		var src_input_element = src_comp ? src_comp._input_element : null;
		if (src_input_element && src_input_element.enable) {
			var is_selected = src_input_element._parent_elem.linkedcontrol._edit_base_api._is_selected();
		...
		if (!pThis.ondragmove || (pThis.ondragmove && !pThis.ondragmove.defaultprevented)) {
			...
			ret = this._on_bubble_dragmove(elem, src_comp, src_refer_comp, dragdata, userdata, button, alt_key, ctrl_key, shift_key, canvasX, canvasY, screenX, screenY, event_bubbles, fire_comp, refer_comp, false, xdeltavalue, ydeltavalue);
		}
	...
	_pComponent._on_bubble_dragmove = function (elem, src_comp, src_refer_comp, dragdata, userdata, button, alt_key, ctrl_key, shift_key, canvasX, canvasY, screenX, screenY, event_bubbles, fire_comp, refer_comp, bubble_scope, xdeltavalue, ydeltavalue) {
		if (!this._is_alive) {
			return;
		}
		...
		if (event_bubbles === undefined) {
			...
			if (this._is_subcontrol) {
				...
				if (this.visible && this._isEnable()) {
					...
					if (bubble_scope) {
						...
						event_bubbles = this.on_fire_sys_ondragmove(src_comp, src_refer_comp, dragdata, userdata, button, alt_key, ctrl_key, shift_key, screenX, screenY, canvasX, canvasY, clientXY[0], clientXY[1], this, refer_comp, xdeltavalue, ydeltavalue);
					}
				}
			}
			...
			if ((!this.ondragmove || (this.ondragmove && !this.ondragmove.stoppropagation)) && event_bubbles !== true && this.parent && !this.parent._is_application) {
				...
				if (is_subcontrol_bubble) {
					return this.parent._on_bubble_dragmove(elem, src_comp, src_refer_comp, dragdata, userdata, button, alt_key, ctrl_key, shift_key, canvasX, canvasY, screenX, screenY, event_bubbles, null, refer_comp, bubble_scope, xdeltavalue, ydeltavalue);
				}
				else if (!this._is_popup_frame || (this._is_popup_frame && !this._is_window)) {
					return this.parent._on_bubble_dragmove(elem, src_comp, src_refer_comp, dragdata, userdata, button, alt_key, ctrl_key, shift_key, canvasX, canvasY, screenX, screenY, false, this, refer_comp, bubble_scope, xdeltavalue, ydeltavalue);
				}
			}
		}
		else {
			if (this.visible && this._isEnable()) {
				...
				if (bubble_scope) {
					...
					event_bubbles = this.on_fire_sys_ondragmove(src_comp, src_refer_comp, dragdata, userdata, button, alt_key, ctrl_key, shift_key, screenX, screenY, canvasX, canvasY, clientXY[0], clientXY[1], fire_comp, refer_comp, xdeltavalue, ydeltavalue);
				}
			}
			if ((this._is_popup_frame && !this._is_window)) {
				return;
			...
			if ((!this.ondragmove || (this.ondragmove && !this.ondragmove.stoppropagation)) && event_bubbles !== true && this.parent && !this.parent._is_application) {
				...
				return this.parent._on_bubble_dragmove(elem, src_comp, src_refer_comp, dragdata, userdata, button, alt_key, ctrl_key, shift_key, canvasX, canvasY, screenX, screenY, false, fire_comp, refer_comp, bubble_scope, xdeltavalue, ydeltavalue);
			}
		}
	};
	...
	_pComponent._on_contextmenu = function (elem, event_bubbles, from_comp, from_refer_comp) {
		...
		if (is_enable) {
			if (nexacro.Browser == "IE" || nexacro.Browser == "Edge") {
				var win = this._getWindow();
	...
	_pComponent.on_fire_sys_ondragmove = function (src_comp, src_refer_comp, dragdata, userdata, button, alt_key, ctrl_key, shift_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp, xdeltavalue, ydeltavalue) {
