...
if (!nexacro.Form) {
	...
	_pBindManager._forceBinds = function () {
		var len = this._form.binds.length;
		for (var sx = 0; sx < len; sx++) {
			var bind_item = this._form.binds[sx];
			this._setBinditem(bind_item, false);
	...
	_pFormBase._on_refresh_stepcontents = function (old_stepcount, old_stepindex, new_stepcount, new_stepindex) {
		...
		if (control_elem) {
			if (this._is_created && (old_stepcount > 0 || new_stepcount > 0)) {
				...
				for (var i = 0; i < comp_len; i++) {
					var comp_elem = comps[i].getElement();
					control_elem.appendChildElement(comp_elem, true);
				}
			}
		}
	...
	_pFormBase.on_fire_sys_onlbuttondown = function (button, alt_key, ctrl_key, shift_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp) {
			...
			if (hotkey_info[1] == keycode && 
				...
				if (obj && comp && comp.parent) {
					if (obj.name != comp.name || obj.parent.name != comp.parent.name) {
						var ctflag = false;
						for (var j = (i + 1), n = hotkey_list.length; j < n; j++) {
				...
				if (edit_base && !edit_base._is_composition()) {
					if (nexacro.OSVersion >= 10 && (nexacro.Browser == "Edge" || nexacro.Browser == "IE")) {
						if (comp instanceof nexacro.TextArea) {
							...
							edit_base.comp._input_element.setElementSetSelect(edit_base._begin_pos, edit_base._end_pos);
						}
						else {
