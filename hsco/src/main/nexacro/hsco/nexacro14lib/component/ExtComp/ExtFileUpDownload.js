	...
	if (!nexacro.ExtFileUpload)
	{
		...
		_pFileUpload.upload = function (v, inDatasetsParam, outDatasetsParam, datatype)
		{
			...
			if (uploadurl)
			{
				...
				//var dsNameArr 	= inDatasetsParam.split(":");
				...
				for (var i = 0; i < len ; i++)
				{
					...
					if (items[i].value)
					{
						...
						//this.loadItem = new nexacro.ExtFileTransaction(uploadurl, "upload", this.parent, dsNameArr[0], outDatasetsParam, datatype);
						
						this.loadItem["parent"] = this;
						this.loadItem["FORM_URL"] = dsNameArr[1];
    ...
    */
	...
	//                }
		...
		_pExtFileUpload.upload = function (path, inDatasetsParam, outDatasetsParam, transferType, datatype)
		{
			...
			//trace("HTMML5 업로드 시작! _pExtFileUpload.upload");
		...
		{
			//trace("HTMML5 업로드 시작! _pExtFileUpload.uploadAll");
			var uploadurl;
			var rtn = false;
...
//				trace("숨기기 2 :" + nMs);					
	...
	if (!nexacro.ExtFileDownload)
	{
		 ...
		 * @param {string} 전달 dataset 정보
		 * @return {boolean} 다운로드 성공여부
		 */  		 
		_pExtFileDownload.download = function (url, filename, inDatasetsParam)
		{
			...
			var hasInDataSet = Eco.isEmpty(inDatasetsParam) ? false : true;
			...
			if (downloadurl)
			{
				...
				var loadItem = null;
				if(hasInDataSet){
					loadItem = new nexacro.ExtFileTransaction(downloadurl, "download", this._form, inDatasetsParam, "", 0);
				}else{
					loadItem = new nexacro.ExtFileTransaction(downloadurl, "download", this._form);
				}
				...
				if (this.support.XHR2 && (this.support.Download || this.support.MSSave))
				{
					...
					if(hasInDataSet){
						...
						formData.append("inputDatasets", loadItem._sendData);
						
						this._startCommunication(loadItem, downloadurl, formData, filename);
		...
		{   
			...
			*/
			...
			var hasSendData = Eco.isEmpty(senddata) ? false : true;
			if(hasSendData){
				method = "POST";        
				mime_xml = true;
			}
				...
				if (this.readyState == 4 && this.status == 200) 
				{
					...
					else if (itemType == "download")
					{
						...
						if (this.responseType == "blob")
						{
							...
							/*
							// 오류구분이 명확하지 않아 주석처리
							var lengthComputable = evt.lengthComputable;

							// 오류 발생시...
							if(lengthComputable == false){
								...
								fr.onload = function (e) {
									...
									var frResult =  e.target.result;

									var str = "";
									var ab = new Uint8Array(frResult);
									var abLen = ab.length;
									...
									responseText = decodeURIComponent(escape(str));

									// String으로 변환 후 처리
									if(responseText != null){

										var fstr = responseText.substring(0, 3);
										...
										{
											//trace("\n\n\t==== HTML5 XML 처리 시작 ===");
											result = _pFileTransaction._deserializeXMLFromStr(responseText);
											code = result[0];
											// 정확한 메시지 출력을 위해 조정
											msg = result[1];// + "[" + status + "," + statusText + "]";
										}

										pThis.on_fire_onerror(pThis, code, msg, this, pFileId, pLoadItem.type);
		...
		_pExtFileDownload.on_fire_onerror = function (obj, errorcode, errormsg, errorobj, fileId, type)
		{
			...
			if(Eco.isEmpty(errormsg)) {
				var sysErrorMsg = nexacro._GetSystemErrorMsg(this, errorcode);
				if(Eco.isEmpty(sysErrorMsg)){
					errorcode = errorobj.status;
					errormsg = errorobj.statusText;
				}
			}  
			...
			if (this.onerror && this.onerror._has_handlers)
			{
				...
				//trace("this.onerror && this.onerror._has_handlers ---------------");
    ...
    _pFileTransaction.on_down_file = function (data, unique_id, cookie, status, statusText)
    {
    	//trace("_pFileTransaction.on_down_file ==============================");
        //trace("on_down_file data.size:" + data.size + ", cookie:" + cookie + ",_downfilename:" + this._downfilename + ", status:" + status + ", statusText:" + statusText);
        var url,
            saveFilename = this._downfilename;
        ...
        if (paramsInfo)
        {
            ...
            while (varInfo)
            {
                ...
                if (id && id.length)
                {
                    ...
                    {
                    	val = nexacro._decodeXml(val);	// Decode추가! 쓰벌!!
                        message = val;
                    }
                    else
                    {
        ...
        if (xml_parse_pos >= -1)
        {
            ...
            while (datasetInfo)
            {
                ...
                if (remoteId && remoteId.length)
                {
					...
					// outDataset 정보가 있을 경우 일반적인 데이터 처리
                    var localId = dsIds[remoteId];
					var ds = null;
					if(localId){
						ds = form._getDatasetObject(localId);
					}else{
						ds = this.getResponseDataset(form);
					}
