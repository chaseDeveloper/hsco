...
if (!nexacro.PopupMenuItem) {
	...
	_pPopupMenuItem.on_fire_sys_onlbuttonup = function (button, alt_key, ctrl_key, shift_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp, from_elem) {
...
if (!nexacro.PopupMenu) {
	...
	_pPopupMenu._on_bubble_mousewheel = function (elem, wheelDeltaX, wheelDeltaY, button, alt_key, ctrl_key, shift_key, canvasX, canvasY, screenX, screenY, event_bubbles, fire_comp, refer_comp, bScroll) {
		if (!this._is_alive) {
			return;
		}
	...
	_pPopupMenu.trackPopupByComponent = function (obj, x, y, align, bcapture) {
		...
		var org_popup_top = alignPosition[1] < 0 ? 0 : alignPosition[1];
	...
	_pPopupMenu._getViewItemQty = function (pThis, _item_length) {
		...
		for (i = pThis._scrollIndex; i < _item_length; i++) {
