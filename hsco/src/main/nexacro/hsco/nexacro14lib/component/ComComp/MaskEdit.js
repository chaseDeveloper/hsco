	...
	_pMaskEdit.on_apply_custom_setfocus = function (evt_name) {
		...
		if (input_elem && api) {
...

			{
				...
				if (nexacro.Browser == "Edge" && (evt_name == "tabkey" || evt_name == "shifttabkey")) {
				...
				if (nexacro.Browser != "Edge" || (nexacro.Browser == "Edge" && evt_name != "tabkey" && evt_name != "shifttabkey")) {
					input_elem.setElementFocus();
				}
	...
	_pMaskEdit._on_input_focus = function (elem, target) {
		...
		if (api) {
			...
			if (this._is_first_focus) {
				if (nexacro.Browser != "Gecko" || (nexacro.Browser == "Gecko" && !api._is_apply_autoselect)) {
					this._input_element.setElementSetSelect(api._begin_pos, api._end_pos);
				}
