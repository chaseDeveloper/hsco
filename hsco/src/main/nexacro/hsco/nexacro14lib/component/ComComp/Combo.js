...
if (!nexacro.Combo) {
	...
	_pCombo.set_value = function (v) {
		if (v != this.value) {
			...
			this.on_apply_value();
			if (this._is_created && this.value === undefined && v !== undefined) {
				this.value = v;
			}
			else {
				this.applyto_bindSource("value", this.value);
	...
	_pCombo.on_notify_item_canitemchange = function (obj, e) {
		...
		if (this.comboedit) {
			...
			if ((this.parent && (this.parent._last_focused && (this.parent._last_focused.id == this.id))) || this._is_subcontrol) {
	...
	_pCombo.on_fire_onkillfocus = function (obj, fromObj) {
		...
		if (this.comboedit) {
			...
			if ((nexacro.Browser == "Runtime" && this.autoselect == true) || nexacro.Browser == "Edge" || nexacro.Browser == "IE") {
	...
	_pCombo._showPopup = function (ds, index) {
		...
		if (combolist) {
			...
			if (nexacro.Browser == "Chrome" && combolist.vscrollbar) {
	...
	_pCombo._getItemValue = function (index) {
		...
		if (ds && column) {
			...
			if (rtn == undefined && (this.type == "filter" || this.type == "filterlike")) {
	...
	_pComboEditCtrl.on_apply_custom_setfocus = function (evt_name) {
		...
		if (input_elem) {
			...
			if (!this.autoselect && this._edit_base_api) {
				if (this._is_set_value && this.value && (evt_name == "tabkey" || evt_name == "shifttabkey" || this.taborder == 0)) {
					this.setCaretPos(this.value.length);
				}
				else if (evt_name != "tabkey" && evt_name != "shifttabkey") {
					this.setCaretPos(this._edit_base_api._begin_pos);
				}
			}
			else if (evt_name == "focus" && this.autoselect) {
				input_elem.setElementSetSelect(0, this.value.length);
			}
