...
if (!nexacro.TextArea) {
	...
	_pTextArea.on_apply_custom_setfocus = function (evt_name) {
		...
		if (input_elem) {
			...
			if (this._old_selection[0] != this._old_selection[1]) {
				input_elem.setElementSetSelect(this._old_selection[0], this._old_selection[1]);
			}
			else if (nexacro.Browser == "Runtime" && evt_name != "lbuttondown" && this._is_set_value) {
				input_elem.setElementSetSelect(this._old_selection[0]);
			}

			input_elem.setElementFocus();

			if (nexacro.Browser == "Runtime" && nexacro.OS == "Windows" && !this._is_set_value) {
			}
			else {
				var h_pos = this.hscrollbar ? this.hscrollbar.pos : 0;
				var v_pos = this.vscrollbar ? this.vscrollbar.pos : 0;

				if (this._is_set_value && (evt_name == "tabkey" || evt_name == "shifttabkey" || this.taborder == 0)) {
					if (!this.autoselect) {
						this.setCaretPos(this.value.length);
					}

					if (nexacro.Browser == "Chrome" || nexacro.Browser == "Opera") {
						if (this.hscrollbar) {
							h_pos = input_elem.getScrollLeft();
						}
						if (this.vscrollbar) {
							v_pos = input_elem.getScrollTop();
						}
					}
					else {
						if (this.hscrollbar) {
							h_pos = input_elem.getScrollWidth();
						}
						if (this.vscrollbar) {
							v_pos = input_elem.getScrollHeight();
						}
					}
				}

				if (!this.autoselect && nexacro.OS != "iOS") {
					if (this._is_first_focus && !this._is_set_value) {
						if (nexacro.Browser == "Chrome" || nexacro.Browser == "Opera") {
							if (this.hscrollbar) {
								this.hscrollbar.set_pos(h_pos);
							}
							if (this.vscrollbar) {
								this.vscrollbar.set_pos(v_pos);
							}
						}
					...
					if (api && evt_name != "tabkey" && evt_name != "shifttabkey" && (nexacro.Browser != "Edge" && nexacro.Browser != "IE" && !(nexacro.OS == "Mac OS" && nexacro.Browser == "Safari"))) {
	...
	_pTextArea.on_hscroll = function (obj, e) {
		...
		if ((nexacro.OS == "iOS" && !nexacro._isHybrid()) || (nexacro.Browser == "IE" || nexacro.Browser == "Edge" || nexacro.Browser == "MobileSafari")) {
			if (nexacro.OSVersion >= 10 && (nexacro.Browser == "Edge" || nexacro.Browser == "IE")) {
		...
		if (v != this.value) {
			...
			if (nexacro.Browser == "Edge" || nexacro.Browser == "IE") {
				...
				if (nexacro.Browser == "Safari") {
					this.setCaretPos(this.text.length);
				}
			}
		...
		if (ret) {
			...
			if (!api._is_composition()) {
				if (nexacro.Browser == "IE" && nexacro.BrowserVersion == 11) {
					...
					if (api._keycode == nexacro.KeyCode_ImeInput && api._stat_composition._curr_data == "" && imetext != "" && api._is_hangul(imetext) && !api._is_on_killfocus) {
						ret = false;
					}
				...
				else if (nexacro._isDesktop() && nexacro.Browser == "Safari" || nexacro.Browser == "Edge") {
					if (api._keycode == nexacro.KeyCode_ImeInput && org_chartext == "" && !api._is_on_killfocus) {
						ret = fireCancharEvent = false;
					}
				}
				else if (nexacro.Browser == "Runtime") {
					var imetext = text_info.postimetext == "" ? text_info.preimetext : text_info.postimetext;
					if (api._keycode == nexacro.KeyCode_ImeInput && org_chartext == "" && (imetext != "" ? !api._is_hangul(imetext) : false)) {
						ret = fireCancharEvent = false;
					}
					if (api._keycode == nexacro.KeyCode_ImeInput && org_chartext == "" && imetext != "" && api._is_hangul(imetext)) {
						ret = fireCancharEvent = false;
					}
							...
							if (bInsert) {
								if (charEvt.posttext != charEvt.pretext && !(api._is_on_killfocus && text_info.pretext == text_info.posttext)) {
									if (charEvt.chartext != "") {
										...
										if (api._keycode == nexacro.Event.KEY_BACKSPACE) {
											ret = this.on_fire_cancharchange(this, text_info.chartext, text_info.prechareventtext, text_info.posttext);
										}
									}
								}
			...
			if (org_chartext != changechartext) {
				...
				else if (nexacro.Browser == "Chrome" && api._is_hangul(org_chartext)) {
