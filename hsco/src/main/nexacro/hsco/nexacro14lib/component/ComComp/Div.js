...
if (!nexacro.Div) {
	...
	_pDiv.on_fire_oninit = function (obj) {
		var _form = this._getForm();
		if (_form && _form != this && _form._is_form && _form._is_init) {
			_form.on_fire_oninit(_form);
		}

		return nexacro.Form.prototype.on_fire_oninit.call(this, obj);
	};
	...
	_pDiv._clear_prop = function () {
		...
		for (var prop in this) {
			if (this.hasOwnProperty(prop) && this[prop] != null && prop != "_registerscriptfn") {
				this[prop] = null;
			}
		}
	...
	_pDiv.on_apply_url = function (reload) {
		if (this._url && this._url.length > 0) {
			...
			if (this._bind_manager) {
				this._bind_manager._forceBinds();
			}
			else {
				var form = this._getRootForm();
				if (form && form._bind_manager) {
					form._bind_manager._forceBinds();
				}
			}
		}
		else {
