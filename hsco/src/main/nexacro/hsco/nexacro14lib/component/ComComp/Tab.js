...
if (!nexacro.Tab) {
	...
	_pTab.on_apply_prop_enable = function (v) {
		...
		if (this.spindownbutton && this._isSpinDownEnable()) {
			this.spindownbutton.set_enable(enable);
		}
	};
	...
	_pTab._on_killfocus = function (new_focus, new_ref_focus) {
		...
		for (i = 0, n = btnborders.length; i < n; i++) {
	...
	_pTab._on_btn_lbuttondown = function (obj, elem, button, alt_key, ctrl_key, shift_key, canvasX, canvasY, screenX, screenY, event_bubbles, fire_comp, refer_comp) {
		...
		if (!pThis.onlbuttondown || (pThis.onlbuttondown && !pThis.onlbuttondown.defaultprevented)) {
			this.on_lbuttondown_default_action(elem, this._focus_refer_comp, canvasX, canvasY, button, alt_key, ctrl_key, shift_key, screenX, screenY);
	...
	_pTab.on_lbuttondown_default_action = function (elem, refer_comp, canvasX, canvasY, button, alt_key, ctrl_key, shift_key, screenX, screenY) {
			...
			if (this.selectchangetype == "down") {
				...
				if (tabbutton_obj instanceof nexacro.TabExtraButtonCtrl) {
					...
					if (idx >= 0) {
						...
						this.on_fire_onextrabuttonclick(tabbutton_obj, idx, button, alt_key, ctrl_key, shift_key, screenX, screenY, canvasX, canvasY, clientXY[0], clientXY[1], this, this);
					}
	...
	_pTab._onclick_basic_action = function (obj, button, alt_key, ctrl_key, shift_key, canvasX, canvasY, screenX, screenY, fire_comp, refer_comp) {
		if (button == "touch" || this.selectchangetype == "up") {
			...
			if (obj instanceof nexacro.TabExtraButtonCtrl) {
				...
				if (idx >= 0) {
					this.on_fire_onextrabuttonclick(obj, idx, button, alt_key, ctrl_key, shift_key, screenX, screenY, canvasX, canvasY, -1, -1, fire_comp, refer_comp);
				}
			}
	...
	_pTab._createTabpage = function (idx, id, obj) {
		...
		for (var i = 0; i < len; i++) {
			if (child_list[i] instanceof nexacro.Grid) {
				...
				child_list[i]._selectRow(row, true);
			}
		}
...
if (!nexacro.TabExtraButtonCtrl) {
	...
	_pTabExtraButtonCtrl._on_lbuttondown = function (elem, button, alt_key, ctrl_key, shift_key, canvasX, canvasY, screenX, screenY, event_bubbles, fire_comp, refer_comp) {
		...
		this.on_lbuttondown_basic_action(elem, button, alt_key, ctrl_key, shift_key, canvasX, canvasY, screenX, screenY, event_bubbles, this, this);
	...
	_pTabExtraButtonCtrl.on_click_basic_action = function (elem, button, alt_key, ctrl_key, shift_key, canvasX, canvasY, screenX, screenY) {
		if (!this._is_alive) {
			return;
		}
		...
		var tab = this.parent;
		if (tab) {
			tab._onclick_basic_action(this, button, alt_key, ctrl_key, shift_key, canvasX + this._adjust_left, canvasY, screenX + this._adjust_left, screenY + this._adjust_top, tab, tab);
		}
		return;
	};
...
if (!nexacro.TabButtonCtrl) {
	...
	_pTabButtonCtrl.on_click_basic_action = function (elem, button, alt_key, ctrl_key, shift_key, canvasX, canvasY, screenX, screenY) {
		if (!this._is_alive) {
			return;
		}
		...
		var tab = this.parent;
		if (tab) {
			tab._onclick_basic_action(this, button, alt_key, ctrl_key, shift_key, canvasX, canvasY, screenX, screenY, tab, tab);
		}
		return;
	};
...
if (!nexacro.TabPageArea) {
	...
	_pTabPageArea.on_find_CurrentStyle_border = function (pseudo) {
		...
		var bordertype = this.parent._find_pseudo_obj("bordertype", pseudo, "bordertype");
		var b = border.clone();
		var position = this.parent._getTabPosition();
...
if (!nexacro.Tabpage) {
	...
	_pTabPage.on_fire_user_onkeydown = function (key_code, alt_key, ctrl_key, shift_key, from_comp, from_refer_comp) {
		...
		return nexacro.Form.prototype.on_fire_user_onkeydown.call(this, key_code, alt_key, ctrl_key, shift_key, from_comp, from_refer_comp);
	};
...
if (!nexacro.TabBorderCtrl) {
		...
		if (this.name == "tableftborder") {
			...
			if (position == "top") {
				...
				if (this.parent._tabButtons && this.parent._tabButtons.length > 0 && this.parent.tabindex > -1) {
					var buttonbackground = this.parent.on_find_CurrentStyle_buttonbackground("selected");
	...
	_pTabBorderCtrl.on_find_CurrentStyle_bordertype = function (pseudo) {
		...
		var b_type = this.parent._find_pseudo_obj("bordertype", this._pseudo, "bordertype");
		if (!b_type) {
			return null;
		}
