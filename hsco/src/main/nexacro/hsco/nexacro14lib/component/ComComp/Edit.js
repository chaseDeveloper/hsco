...
if (!nexacro.Edit) {
	...
	_pEdit.on_created_contents = function () {
		...
		if (input_elem) {
			...
			if (nexacro._isDesktop() && (nexacro.Browser == "Chrome")) {
				this.setCaretPos(0);
			}
	...
	_pEdit.on_apply_custom_setfocus = function (evt_name) {
		...
		if (input_elem) {
			...
			if (nexacro.Browser == "Gecko" || nexacro.Browser == "Safari") {
			...
			if (!this.autoselect && nexacro.OS != "iOS") {
				...
				if (evt_name == "tabkey" || evt_name == "shifttabkey" || this.taborder == 0) {
					if (this._is_set_value) {
						this.setCaretPos(this.value.length);
					}
					else if (nexacro.Browser == "Edge" && !api._is_selected()) {
						_flag = true;
					}
				}
				else if (nexacro.Browser != "Edge" && nexacro.Browser != "Runtime" && !(nexacro.OS == "Mac OS" && nexacro.Browser == "Safari")) {
					_flag = true;
				}
			...
			var pThis = this;
			if (nexacro.OS == "iOS" && input_elem._keypad_type == "change" && !this.readonly) {
				setTimeout(function () {
					api._text = text;
					input_elem.setElementInputType("text", true);
					...
					input_elem.setElementInputTypeKeypad("changed");
				}, 500);
			}
			this._is_set_value = false;
		}
