...
if (!nexacro.Calendar) {
	...
	_pCalendar.on_apply_custom_setfocus = function (evt_name) {
		...
		if (edit) {
			...
			if (edit_api && !this._re_focus) {
				...
				if (!edit._input_element._is_mousedown && !(nexacro.Browser == "Runtime" && evt_name == "lbuttondown")) {
					if (nexacro.Browser == "Edge" && this.autoselect) {
						this._caret_pos.begin = 0;
						this._caret_pos.end = this.value.length;
					}
		...
		if (propid == "value") {
			...
			if (col_val) {
				if (col_val instanceof nexacro.Date) {
					...
					var v = this._makeDateObj(col_val);
	...
	_pCalendar._on_killfocus = function (new_focus, new_ref_focus) {
		...
		if (this.calendaredit) {
			this._caret_pos.begin = this.calendaredit._edit_base_api._begin_pos;
			this._caret_pos.end = this.calendaredit._edit_base_api._end_pos;
		}

		if (this.calendaredit && nexacro.Browser == "Runtime") {
			this.calendaredit._cancelSelect();
		}
	...
	_pCalendar._getDateObjFromDisplayText = function (edit_value) {
		...
		if (this.value) {
			...
			if (timeformat) {
				if (hour == "00" && this._editformat_info._hour == null) {
					hour = this.value.getHours();
				}

				if (min == "00" && this._editformat_info._min == null) {
					min = this.value.getMinutes();
				}

				if (sec == "00" && this._editformat_info._sec == null) {
	...
	_pCalendar._setValue = function (v, user_set) {
		...
		if (this._bindsource) {
			...
			if (this._bind_event) {
				...
				if (_binditem && this.value && date_obj && date_obj.valueOf() != this.value.valueOf()) {
					this._currentformat = currentformat;
					_binditem = null;
					return ret;
				}
	...
	_pCalendarEditCtrl.set_value = function (v, apply_fake) {
		...
		if (v != this.value) {
			...
			if (this._is_first_focus || (this.value != v)) {
