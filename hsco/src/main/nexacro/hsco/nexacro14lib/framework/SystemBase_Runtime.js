...
if (nexacro.Browser == "Runtime") {
	if (!nexacro.Init_systembase_runtime) {
		...
		nexacro._TextInfoCacheManager = function (font, letterspace, rtldirection) {
			this.font = font;
		...
		_pTextInfoCacheManager.getWordWrapTextSize = function (letterspace, text, width, wordwrap, rtldirection) {
			...
			if (text_width == null) {
				var font = this.font;
				var size = nexacro.__getTextSize(letterspace, text, font.face, font.size, font._antialias, font._bold, font._italic, font._underline, font._strikeout, wordwrap, width, rtldirection);
		...
		(function () {
			...
			nexacro._getWordwrapTextSize = function (manager, letterspace, text, content_width, wordwrap, rtldirection) {
				...
				var text_size = manager.getWordWrapTextSize(letterspace, text, content_width, wordwrap, rtldirection);
				return text_size;
			};
		})();

		nexacro._getTextSize = function (letterspace, text, font, multiline, content_width, wordwrap, rtldirection) {
			if (text && text.length > 0 && font && !font._is_empty) {
				var cacheManager = nexacro._TextSizeCacheManagers[font._sysvalue];
				if (cacheManager == null) {
					cacheManager = new nexacro._TextInfoCacheManager(font, letterspace, rtldirection);
					nexacro._TextSizeCacheManagers[font._sysvalue] = cacheManager;
				}

				if (multiline) {
					if (content_width != null) {
						return nexacro._getWordwrapTextSize(cacheManager, letterspace, text, content_width, wordwrap, rtldirection);
					}
					else {
		...
		nexacro.getTextSize = function (text, font, width, wordwrap, letterspace, rtldirection) {
			...
			var retn = nexacro._getTextSize(letterspace, text, font, line, width, wordwrap, rtldirection);
			var obj = {
				nx : retn[0], 
				ny : retn[1]
			};
		...
		nexacro._setBrowserErrorMsg = function () {
			var errorcode = ["comm_http_400", "comm_http_404", "comm_http_405", "comm_http_500", "comm_http_503", "comm_http_499", "comm_http_599", "comm_http_401", "comm_http_402", "comm_http_403", "comm_http_406", "comm_http_407", "comm_http_408", "comm_http_301", "comm_http_302", "comm_http_305", "comm_http_307", "native_notexist_licensefile", "native_expire_license", "native_invalid_licensefile", "native_license_invaliddomain", "native_license_fail_parse", "native_license_invalidservice", "msg_access_invalid_file", "msg_access_invalid_domain", "msg_permit_folder", "msg_permit_domain", "range_argument", "msg_elevation_required"
			];
