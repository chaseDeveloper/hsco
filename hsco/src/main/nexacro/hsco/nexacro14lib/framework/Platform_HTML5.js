...
if (nexacro.Browser != "Runtime") {
	if (!nexacro._init_platform_HTML5) {
		...
		if (nexacro.Browser == "Edge") {
			nexacro.SupportTouch = ("ontouchstart" in window || (window.navigator.pointerEnabled && (window.navigator.maxTouchPoints > 1)));
		}
		else {
			...
			nexacro._syshandler_onmouseup = function (_sysEvent, node, evt) {
				...
				if (win && elem) {
					if (evt.button == (nexacro._is_attach_childframe ? nexacro_HTMLSysEvent[id].MOUSE_LBUTTON : nexacro_HTMLSysEvent.MOUSE_LBUTTON)) {
						...
						ret = win._on_sys_rbuttonup(elem, "rbutton", evt.altKey, evt.ctrlKey, evt.shiftKey, evt.clientX, evt.clientY, evt.screenX, evt.screenY);
					}
					else {
						ret = win._on_sys_mouseup(elem, "mbutton", evt.altKey, evt.ctrlKey, evt.shiftKey, evt.clientX, evt.clientY, evt.screenX, evt.screenY);
					}
					var last_focused_elem = win._last_focused_elem;
					if (last_focused_elem && (win._last_focused_elem instanceof nexacro.InputElement) && last_focused_elem != elem) {
						...
						if (comp && comp._edit_base_api) {
							comp._edit_base_api._on_input_mouseup(last_focused_elem, evt.altKey, evt.ctrlKey, evt.shiftKey);
						}
						comp = null;
					}
		...
		nexacro.__setViewportScale = function () {
			...
			if (ratio == 1.0) {
				if (use_autozoom === false) {
					...
					if (nexacro.Browser == "Chrome" && nexacro.BrowserVersion > 38) {
