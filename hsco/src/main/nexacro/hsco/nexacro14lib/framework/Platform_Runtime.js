...
if (nexacro.Browser == "Runtime") {
	if (!nexacro._init_platform_runtime) {
		...
		nexacro._syshandler_onlbuttonup_forward = function (_window, elem, strButton, altKey, ctrlKey, shiftKey, windowX, windowY, screenX, screenY) {
			...
			ret = _window._on_sys_lbuttonup(elem, strButton, altKey, ctrlKey, shiftKey, windowX, windowY, screenX, screenY);

			var firecur_comp = _window._findComponentForEvent(elem);
			if (firecur_comp && firecur_comp.usedecorate) {
			...
			if (last_focused_elem && (_window._last_focused_elem instanceof nexacro.InputElement) && last_focused_elem != elem) {
				var comp = null;
				...
				if (comp && comp._edit_base_api) {
					comp._edit_base_api._on_input_mouseup(last_focused_elem, altKey, ctrlKey, shiftKey);
				}
				comp = null;
			}
