...
if (!nexacro._bInitPlatform) {
				...
				case 0:
					...
					if (info) {
						...
						if (Math.abs(slideinfo.xdelta) <= this.unintendedmove_threshold && Math.abs(slideinfo.ydelta) <= this.unintendedmove_threshold) {
							...
							if (sh && (slideinfo.xdelta > 0 && sh.pos == 0 || slideinfo.xdelta < 0 && sh.pos == sh.max)) {
								return true;
							}
							else if (sh && (slideinfo.xdelta > 0 && sh.pos > 0 || slideinfo.xdelta < 0 && sh.pos < sh.max)) {
								return false;
							}
						}
						else if (Math.abs(slideinfo.xdelta) <= this.unintendedmove_threshold && Math.abs(slideinfo.ydelta) > this.unintendedmove_threshold) {
							if (sv && (slideinfo.ydelta > 0 && sv.pos == 0 || slideinfo.ydelta < 0 && sv.pos == sv.max)) {
								return true;
							}
							else if (sv && (slideinfo.ydelta > 0 && sv.pos > 0 || slideinfo.ydelta < 0 && sv.pos < sv.max)) {
								return false;
							}
						}
					}
	...
	_pTouchManager.ontouchmove = function (win, elem, touchid, clientX, clientY, screenX, screenY, curtime, orgtime, is_last_changedtouch) {
		...
		if (start_comp && curr_comp) {
			if (start_comp != curr_comp) {
				...
				start_comp._stat_change(this._start_elem_status, this._start_elem_pseudo);
			}
		}
				...
				if (!dragInfo.isDragging) {
					if (windowX != dragInfo.startX || windowY != dragInfo.startY) {
						...
						if (ret && ret[0] === true) {
							...
							comp[0]._on_dragmove(elem, dragInfo.target, dragInfo.referTarget, dragInfo.data, dragInfo.userdata, "touch", false, false, false, canvasX, canvasY, screenX, screenY, undefined, undefined, undefined, xdeltavalue, ydeltavalue);
		...
		if (!this._slide_blocked) {
			if (is_slide) {
				...
				if (this._is_ScrollEnd(slideinfo) || this._is_ScrollEnd(slideinfo) === undefined) {
					return false;
				}
	...
	_pTouchManager.ontouchend = function (win, elem, touchid, clientX, clientY, screenX, screenY, curtime, orgtime, is_last_changedtouch) {
		...
		if (touchlen == 1) {
			...
			else if ((comp && compinfo) && (comp[0] != compinfo[0])) {
	...
	_pLayoutManager.checkValid = function (form, szContainer) {
		...
		for (var i = 0; i < layout_len; i++) {
			...
			if (layout.width > szContainer.cx || layout.height > szContainer.cy) {
				if (tmp_width > layout.width) {
					tmp_correct = i;
					tmp_width = layout.width;
				}
				else if (tmp_height > layout.height) {
					tmp_correct = i;
						...
						if (!_is_track) {
							if (nexacro.SupportTouch) {
								...
								if (manager) {
									...
									if (Math.abs(gapX) <= manager.unintendedmove_threshold && Math.abs(gapY) <= manager.unintendedmove_threshold) {
				...
				if (windowX != dragInfo.startX || windowY != dragInfo.startY) {
					...
					if (comp && comp[0]) {
						...
						if (ret && ret[0] === true && nexacro._cur_drag_info) {
							...
							dragInfo.data = ret[3];
							dragInfo.userdata = ret[4];
							comp[0]._on_dragmove(elem, dragInfo.target, dragInfo.referTarget, dragInfo.data, dragInfo.userdata, strButton, altKey, ctrlKey, shiftKey, canvasX, canvasY, screenX, screenY, undefined, undefined, undefined, xdeltavalue, ydeltavalue);
							var c = comp[0];
							var is_scroll = false;
	...
	_pWindow._on_default_sys_deactivate = function () {
		...
		if (this._is_alive && !(this._last_focused_elem instanceof nexacro.WebBrowserPluginElement)) {
	...
	if (!this.application) {
		...
		_pApplication._addcachequerystring = function (url, type) {
			var qrystr = "?version=";
			if (type == "appversion") {
				qrystr += (application.version || application._appversion);
			}
			else {
		...
		_pApplication.loadCss = function (url) {
			...
			if (!this.checkversion && !this._appversion) {
				cssurl.push(".js");
			}
		...
		_pApplication._transurl = function (baseurl, typedefintionurl, url) {
			...
			else if (application._appversion) {
				exturl += ".js?version=" + application._appversion;
			}
		...
		_pApplication._getFDLLocation = function (fdlurl, baseurl) {
			...
			if (url.length > 5) {
				if (url.substring(url.length - 5) == ".xfdl") {
					url = url + ".js" + (this.checkversion ? this._addcachequerystring(url, "serviceversion") : "");
				}
			}
			return url;
		};
