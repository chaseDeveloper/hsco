...
var pop_rolling2 = function(popContainer) {
	// 시간단위는 ms로 1000이 1초
	if (popContainer.nodeType == 1) {
		this.popContainer = popContainer;
	} else {
...
pop_rolling2.prototype.init = function() {
	var pop_children = this.popContainer.childNodes;
	for (var i=(pop_children.length-1); 0<=i; i--) {
		if (pop_children[i].nodeType != 1) {
			this.popContainer.removeChild(pop_children[i]);
...
pop_rolling2.prototype.moveAt = function(oBtn) {
	var i = oBtn.id.substring(12);
	this.mouseover = true;
	if (!this.time_timer_pause) {
		this.counter = (i-1);
...
pop_rolling2.prototype.move_right = function() {
	var oRoll = this;
	var nTemp = 0;
	for (var i=0, m=oRoll.pop_children.length; i<m; i++) {
		nTemp = 0;
...
pop_rolling2.prototype.start = function() { //롤링 시작
	var oRoll = this;
	this.stop();
	this.is_rolling = true;
...
pop_rolling2.prototype.pause = function() { //일시 멈춤
	this.is_rolling = false;
}

pop_rolling2.prototype.resume = function() { //일시 멈춤 해제
	this.is_rolling = true;
}

pop_rolling2.prototype.stop = function() { //롤링을 끝냄
	this.is_rolling = false;
	if (!this.time_timer) {
		clearInterval(this.time_timer);
	}
